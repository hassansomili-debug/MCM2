...
async function loadDashboard() {
  const participantRoute = window.location.hash.match(/^#participant-assessment\/(\d+)$/);
  const participantToken = sessionStorage.getItem('mcm_participant_token');
  if (participantRoute && participantToken) return renderParticipantAssessment(participantToken);
...
function renderParticipantEntry() { document.querySelector('#app-view').innerHTML = `<section class="page-heading"><div><div class="eyebrow">نضج MCM <span>•</span> بوابة المشارك</div><h1>ابدأ تقييمك من هنا</h1><p>ابدأ التقييم مباشرة دون تسجيل أو دعوة.</p></div></section><section class="upload-panel" style="max-width:620px"><button class="primary-button" id="start-participant" type="button">ابدأ التقييم الآن</button><div id="participant-entry-result" class="import-result" hidden></div></section>`; document.querySelector('#start-participant').addEventListener('click', async (event) => { const button = event.currentTarget; button.disabled = true; button.textContent = 'جارٍ تجهيز التقييم...'; try { const session = await api('/api/participant/start', { method: 'POST', body: '{}' }); sessionStorage.setItem('mcm_participant_token', session.token); window.location.hash = `participant-assessment/${session.assessment_id}`; renderParticipantAssessment(session.token); } catch (error) { button.disabled = false; button.textContent = 'ابدأ التقييم الآن'; showToast(error.message); } }); }
async function renderParticipantAssessment(token) { try { const data = await api(`/api/participant/session/${encodeURIComponent(token)}`); document.querySelector('#app-view').innerHTML = `<section class="page-heading"><div><div class="eyebrow">بوابة المشارك <span>•</span> تقييم مباشر</div><h1>التقييم المؤسسي</h1><p>تحفظ إجاباتك تلقائيًا على الخادم.</p></div><button class="primary-button" id="finish-participant" type="button">إنهاء التقييم</button></section><form id="participant-assessment-form" class="section-block">${data.items.map((item) => `<label class="assessment-item"><span>${item.code} · ${item.prompt_ar}</span><input type="number" min="0" max="5" step="1" name="${item.id}" value="${data.answers?.[item.id] ?? ''}" required></label>`).join('')}</form>`; let timer; document.querySelector('#participant-assessment-form').addEventListener('input', () => { clearTimeout(timer); timer = setTimeout(() => saveParticipantAnswers(token), 500); }); document.querySelector('#finish-participant').addEventListener('click', async () => { await saveParticipantAnswers(token); showToast('تم حفظ إجاباتك'); }); } catch (error) { showToast('تعذر تحميل التقييم'); } }
async function saveParticipantAnswers(token) { const answers = [...document.querySelectorAll('#participant-assessment-form input')].filter((input) => input.value !== '').map((input) => ({ item_id: Number(input.name), value: Number(input.value) })); if (answers.length) await api(`/api/participant/session/${encodeURIComponent(token)}/answers`, { method: 'POST', body: JSON.stringify({ answers }) }); }
