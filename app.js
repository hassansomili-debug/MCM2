...
const roleLabels = {
  COMPANY_RESPONDENT: t('مشارك شركة'), COMPANY_ADMIN: t('مدير شركة'), CONSULTANT: t('استشاري'),
  RESEARCHER: t('باحث'), SUPER_ADMIN: t('مدير المنصة'),
};
const statusLabels = {
  DRAFT: t('مسودة'), IN_PROGRESS: t('قيد التنفيذ'), COMPLETED: t('مكتمل'), PILOT: t('قيد التجربة الداخلية'),
  VALIDATED: t('مُعتمد'), ARCHIVED: t('مؤرشف'), PENDING: t('بانتظار القبول'), ACCEPTED: t('مقبولة'),
  ACTIVE: t('نشط'), INACTIVE: t('موقوف'),
  EVIDENCE_INFORMED: t('قائم على الأدلة'), EXPERT_REVIEWED: t('مراجَع من خبراء'), EMPIRICALLY_VALIDATED: t('محقق تجريبيًا'),
  NOT_STARTED: t('لم يبدأ'), IN_PROGRESS_ROADMAP: t('قيد التنفيذ'), DEFERRED: t('مؤجل'),
};
const errorLabels = {
  invalid_credentials: t('بيانات الدخول غير صحيحة.'), session_expired: t('انتهت الجلسة. سجّل الدخول مرة أخرى.'),
  authentication_required: t('يلزم تسجيل الدخول.'), permission_denied: t('ليست لديك صلاحية لهذا الإجراء.'),
  required_answers_missing: t('لا يزال هناك بنود مطلوبة بلا إجابة.'), assessment_locked: t('هذا التقييم مكتمل ومقفل.'),
  password_policy_failed: t('استخدم 10 أحرف على الأقل تتضمن حرفًا كبيرًا وصغيرًا ورقمًا.'),
  email_already_exists: t('البريد مستخدم بالفعل.'),
  instrument_validation_failed: t('ملف الأداة لم يجتز التحقق.'), rate_limit_exceeded: t('محاولات كثيرة. حاول لاحقًا.'),
  internal_error: t('حدث خطأ غير متوقع. حاول مرة أخرى.'), route_not_found: t('المسار المطلوب غير موجود.'),
  registration_disabled: t('إنشاء الحسابات معطل في بيئة العرض المؤقتة.'),
  service_consent_required: t('يلزم قبول موافقة الخدمة للبدء.'), organization_name_required: t('أدخل اسم منشأة صحيحًا.'),
  sector_required: t('اختر قطاع المنشأة.'), sme_size_required: t('اختر حجم منشأة صغيرة أو متوسطة.'),
  region_required: t('اختر المنطقة.'), respondent_role_required: t('اختر دور المجيب.'),
  invalid_demographics: t('تحقق من عدد الموظفين وعمر المنشأة وحجم فريق التواصل.'),
  business_model_required: t('اختر نموذج أعمال المنشأة.'), regulated_sector_required: t('حدد ما إذا كان القطاع منظمًا رقابيًا.'),
  platform_not_ready: t('المنصة قيد التهيئة؛ حاول بعد قليل.'), durable_storage_required: t('التقييم المباشر متوقف مؤقتًا حتى يتم ربط قاعدة بيانات دائمة. تواصل مع مسؤول المنصة.'),
  contact_consent_required: t('يلزم إقرار موافقة التواصل لإرسال الطلب.'),
  contact_consent_withdrawn: t('سُحبت موافقة التواصل؛ لا يمكن إلا الإغلاق أو الإيقاف.'),
  consultant_not_eligible: t('المستشار غير مؤهل أو غير نشط.'),
  unsupported_consultation_status: t('حالة غير مدعومة.'),
  note_required: t('أدخل نص الملاحظة.'),
  invalid_filter: t('قيمة فلتر غير صالحة.'),
  invalid_phone_number: t('تحقق من رقم الجوال؛ استخدم صيغة مثل 05xxxxxxxx أو +9665xxxxxxxx.'),
  full_name_required: t('أدخل الاسم الكامل.'),
  unsupported_contact_method: t('وسيلة التواصل المختارة غير مدعومة.'),
  unsupported_consultation_topic: t('أحد محاور الاستشارة غير مدعوم.'),
  invalid_consultation_topics: t('تعذر قراءة محاور الاستشارة.'),
  consultation_lead_not_found: t('الطلب غير متاح لهذا الحساب.'),
  assessment_id_required: t('تعذر ربط الطلب بالتقييم.'),
  participant_session_invalid: t('انتهت جلسة المشارك؛ افتح النتيجة من جديد.'),
  cannot_delete_own_account: t('لا يمكنك حذف حسابك أنت.'),
  last_platform_administrator: t('لا يمكن حذف آخر مدير منصة نشط.'),
  user_not_found: t('المستخدم غير موجود.'),
  join_code_required: t('أدخل كود المشاركة.'),
  join_code_invalid_or_expired: t('كود المشاركة غير صالح أو منتهي أو استُنفد.'),
  service_consent_required: t('يلزم إقرار موافقة الخدمة.'),
  organization_name_required: t('أدخل اسم المنشأة.'),
  assessment_cooldown_active: t('لا يمكن بدء تقييم جديد قبل انقضاء دورة القياس.'),
  join_code_unavailable: t('تعذر توليد كود؛ حاول مرة أخرى.'),
};
...
function modelNotice() {
  ...
  return `<div class="research-notice"><strong>${e(model.label_ar || t('نموذج تطبيقي قائم على الأدلة العلمية'))}</strong><span>${e(pick(model, 'result_disclaimer_ar').replace(/^$/, model.result_disclaimer_ar || ''))}</span></div>`;
}
...
async function renderLanding() {
  ...
  const model = modelInfo();
  const storageNotice = state.publicConfig.storage === 'ephemeral-demo' ? `<div class="landing-alert"><strong>${t('نسخة عرض مؤقتة')}</strong><span>${e(state.publicConfig.notice || t('قد تُعاد تهيئة البيانات؛ لا تدخل بيانات حساسة.'))}</span></div>` : '';
  ...
  appView.innerHTML = `<div class="landing-page">${publicNav()}${storageNotice}<main>
    <section class="landing-hero">
      <div class="hero-copy">
        ...
        <h1>${t('اعرف أين تقف منشأتك، وحوّل الاتصال إلى')} <i>${t('قدرة مؤسسية تصنع الأثر.')}</i></h1>
        <p>${t('شخّص نضج الاتصال التسويقي، واكتشف مدى تحوّله إلى كفاءة في التواصل الاجتماعي، ثم انتقل من النتيجة إلى أولويات وخارطة تحسين قابلة للتنفيذ.')}</p>
      ...
      <aside class="hero-rationale" aria-label="أهمية النضج الاتصالي التسويقي">
        ...
        <p><strong>${t('النضج الاتصالي ليس نشاطًا تسويقيًا، بل قدرة مؤسسية.')}</strong> ${t('هو أن تكون الرسالة محكومة بمالك واضح، والمعلومة من مصدر واحد معتمد، ورحلة العميل متسقة بين الإدارات، والقرار مبنيًا على أدلة تُراجع دوريًا.')}</p>
        <p>${t('حين ترسخ هذه القدرة تقلّ التناقضات أمام العميل، ويقصر زمن القرار، ويتراجع الهدر في الجهد والإنفاق التسويقي، لأن المبادرات تصبح مرتبطة بهدف ومؤشر بدل أن تتفرق باجتهادات فردية.')}</p>
        <p class="hero-rationale-model">${t('ويقترح النموذج أن ارتفاع هذه القدرة ينعكس على كفاءة الاتصال عبر وسائل التواصل الاجتماعي. المقياس يقيس الاثنين بصورة مستقلة ويعرض الفجوة بينهما، دون ادعاء علاقة سببية مثبتة.')}</p>
        <small>${e(model.label_ar || t('نموذج تطبيقي قائم على الأدلة العلمية'))}</small>
      </aside>
    </section>
    <section class="landing-confidence" aria-label="ما الذي يتغير عند تحسين النضج الاتصالي">
      <article><b>${t('اتساق الرسالة')}</b><span>${t('رسالة واحدة عبر كل قناة ونقطة تماس، بدل إجابات تختلف باختلاف الموظف.')}</span></article>
      <article><b>${t('سرعة القرار')}</b><span>${t('معلومة موثقة ومالك معروف، فيقصر زمن الاستجابة والتصعيد.')}</span></article>
      <article><b>${t('كفاءة الإنفاق')}</b><span>${t('مبادرات مرتبطة بهدف ومؤشر ومراجعة، بدل جهد متفرق يصعب قياس أثره.')}</span></article>
      <article><b>${t('قدرة لا تعتمد على الأفراد')}</b><span>${t('ممارسات راسخة في النظام تصمد مع النمو وتغيّر الفريق.')}</span></article>
    </section>
    <section class="landing-section value-section">
      <div class="section-intro"><span>${t('أبعد من مؤشرات التفاعل')}</span><h2>${t('ماذا سيكشف المقياس لمنشأتك؟')}</h2><p>${t('قراءة إدارية تساعدك على تحديد موضع القدرة الاتصالية، وفهم الفجوات، وترتيب الاستثمار التالي.')}</p></div>
      <div class="value-grid">
        <article><b>01</b><h3>${t('أين تقف الآن؟')}</h3><p>${t('تصنيف المنشأة ضمن خمس مراحل واضحة، من العمل العشوائي إلى القدرة المؤسسية الذكية.')}</p></article>
        <article><b>02</b><h3>${t('ما الذي يعطّل الكفاءة؟')}</h3><p>${t('فهم الفجوة بين قدرات المؤسسة وبين سرعة وجودة وكلفة الاتصال عبر المنصات.')}</p></article>
        <article><b>03</b><h3>${t('ما الخطوة التالية؟')}</h3><p>${t('أولويات مرتبة وخارطة عمل مرحلية مرتبطة بالمسؤول والمؤشر والأفق الزمني.')}</p></article>
      </div>
    </section>
    <section class="landing-section model-section" id="methodology-public">
      <div class="section-intro"><span>${t('النموذج التشخيصي')}</span><h2>${t('النضج قدرة مؤسسية، والكفاءة نتيجة اتصالية مقترحة')}</h2><p>${t('تُحفظ خصائص المنشأة والممكنات كسياق تفسيري منفصل، ولا تُخلط بدرجة النضج الأساسية.')}</p></div>
      <div class="construct-grid">
        ...
        <div class="effect-card"><b>${t('أثر إيجابي')}<br>${t('مقترح')}</b><span aria-hidden="true">←</span><small>${t('يُختبر كميًا ولا يُعرض كعلاقة سببية مثبتة')}</small></div>
    ...
    <section class="landing-section stages-section" id="maturity-public">
      <div class="section-intro"><span>${t('مسار التطور')}</span><h2>${e(maturity.title_ar || t('مراحل النضج الاتصالي التسويقي'))}</h2><p>${e((locale() === 'en' && maturity.subtitle_en) ? maturity.subtitle_en : (maturity.subtitle_ar || ''))}</p></div>
      ${levels.length ? `<ol class="landing-maturity" aria-label="مراحل النضج الخمس">${maturityCards}</ol>` : emptyState(t('تعذر تحميل المراحل'),t('حدّث الصفحة للمحاولة مرة أخرى.'))}
    ...
    <section class="landing-section report-preview">
      <div><span>${t('نتيجة قابلة للاستخدام')}</span><h2>${t('نتائج تنفيذية، وليست مجرد درجة.')}</h2><p>${t('يقارن بين MCM وSMCE، يعرض الأبعاد والممكنات والنتائج، يحدد المرحلة الحالية، ثم يرتب فرص التطوير. وتتوفر للمدير ملفات Excel وSPSS مع قاموس متغيرات وترميزات جاهزة.')}</p><a class="primary-button large" href="#participant-start">${t('ابدأ تشخيص منشأتك')}</a></div>
      <div class="preview-card"><header><span>${t('نتيجة افتراضية للتوضيح')}</span><b>${e(levels[2]?.label_ar || t('مدار / متكامل'))}</b></header><div class="preview-score"><strong>64</strong><small>/100</small></div><div class="preview-bars">${[82,71,64,58,46].map((score,index)=>`<div><span>البعد ${index+1}</span><i><b style="--score:${score}%"></b></i><em>${score}</em></div>`).join('')}</div><footer><span>${t('الأولوية التالية')}</span><b>${t('تعزيز التعلّم والتكيّف')}</b></footer></div>
    </section>
    <section class="landing-cta"><span>${t('البدء لا يحتاج حسابًا')}</span><h2>${t('أدخل معلومات منشأتك وابدأ الإجابة مباشرة.')}</h2><p>${t('خيارات لفظية واضحة بمقياس ليكرت، مع «لا ينطبق» و«لا أعرف» عند الحاجة.')}</p><a class="primary-button light large" href="#participant-start">${t('ابدأ المقياس الآن')}</a></section>
  </main><footer class="public-footer"><b>${t('مقياس النضج الاتصالي التسويقي')}</b><span>${e(model.label_ar || t('نموذج تطبيقي قائم على الأدلة العلمية'))} للمنشآت الصغيرة والمتوسطة · 2026</span><a href="#login">${t('تسجيل الدخول')}</a></footer></div>`;
}

function renderParticipantStart() {
  state.publicPage = true; applyShell();
  if (!state.publicConfig.direct_participant_enabled) {
    appView.innerHTML = `<div class="participant-start-page">${publicNav()}<main><section class="route-state">${emptyState(t('التقييم المباشر غير متاح حاليًا'), state.publicConfig.notice || t('يجب ربط قاعدة بيانات دائمة قبل استقبال التقييمات.'), '<a class="primary-button" href="#landing">العودة للرئيسية</a>')}</section></main></div>`;
    return;
  }
  ...
  const storageNotice = state.publicConfig.storage === 'ephemeral-demo' ? `<div class="research-notice"><strong>${t('تنبيه بيئة العرض')}</strong><span>${e(state.publicConfig.notice || t('البيانات مؤقتة؛ استخدم معلومات غير حساسة فقط.'))}</span></div>` : '';
  appView.innerHTML = `<div class="participant-start-page">${publicNav()}<main><section class="start-heading"><div><span>${t('دخول مباشر للمشارك')}</span><h1>${t('عرّفنا بمنشأتك قبل بدء المقياس')}</h1><p>${t('تساعد هذه المتغيرات على تفسير النتيجة في سياق المنشأة، ولا تدخل في درجة النضج الأساسية. ستقيّم الممكنات التنظيمية داخل الأداة نفسها.')}</p></div><div class="start-meta"><b>${t('حوالي 15–20 دقيقة')}</b><span>${t('لا يلزم إنشاء حساب')}</span><span>${t('الحفظ تلقائي')}</span></div></section>${storageNotice}<form id="direct-assessment-form" class="direct-form"><section class="panel"><div class="form-section-title"><b>01</b><div><h2>${t('الخصائص الديموغرافية للمنشأة')}</h2><p>${t('تُحفظ مع حالة القياس لأغراض التشخيص والتحليل المجهول وفق الموافقات.')}</p></div></div><div class="form-grid"><label class="field full"><span>${t('اسم المنشأة')}</span><input name="organization_name" minlength="2" maxlength="120" required></label><label class="field"><span>${t('القطاع')}</span><select name="sector" required><option value="">${t('اختر القطاع')}</option><option>${t('التجزئة والتجارة')}</option><option>${t('الخدمات المهنية')}</option><option>${t('التقنية والاتصالات')}</option><option>${t('الصناعة')}</option><option>${t('السياحة والضيافة')}</option><option>${t('الصحة')}</option><option>${t('التعليم')}</option><option>${t('النقل والخدمات اللوجستية')}</option><option>${t('قطاع آخر')}</option></select></label><label class="field"><span>${t('حجم المنشأة')}</span><select name="firm_size" required><option value="">${t('اختر الحجم')}</option><option value="MICRO">${t('متناهية الصغر')}</option><option value="SMALL">${t('صغيرة')}</option><option value="MEDIUM">${t('متوسطة')}</option></select></label><label class="field"><span>${t('عدد الموظفين')}</span><input name="employee_count" type="number" min="1" max="250000" required></label><label class="field"><span>${t('عمر المنشأة بالسنوات')}</span><input name="firm_age_years" type="number" min="0" max="200" required></label><label class="field"><span>${t('نموذج الأعمال')}</span><select name="business_model" required><option value="">${t('اختر النموذج')}</option><option value="B2C">${t('منشأة إلى مستهلك B2C')}</option><option value="B2B">${t('منشأة إلى منشأة B2B')}</option><option value="B2B2C">${t('مختلط B2B2C')}</option><option value="GOVERNMENT">${t('جهة حكومية / تعامل حكومي')}</option><option value="NONPROFIT">${t('غير ربحي')}</option><option value="OTHER">${t('آخر')}</option></select></label><label class="field"><span>${t('المنطقة الرئيسية للنشاط')}</span><select name="region" required><option value="">${t('اختر المنطقة')}</option>${regions.map(region=>`<option>${e(region)}</option>`).join('')}</select></label><label class="field"><span>${t('عدد منصات التواصل المستخدمة بانتظام')}</span><input name="social_platform_count" type="number" min="0" max="50" required></label><label class="field"><span>${t('عدد العاملين المسؤولين مباشرة عن التواصل الاجتماعي')}</span><input name="social_team_size" type="number" min="0" max="250000" required></label><label class="field"><span>${t('هل تعمل المنشأة في قطاع منظم رقابيًا؟')}</span><select name="regulated_sector" required><option value="">${t('اختر الإجابة')}</option><option value="YES">${t('نعم')}</option><option value="NO">${t('لا')}</option></select></label><label class="field full"><span>${t('دور المجيب')}</span><select name="respondent_role" required><option value="">${t('اختر الدور')}</option><option>${t('مالك / مؤسس')}</option><option>${t('إدارة عليا')}</option><option>${t('مدير تسويق أو اتصال')}</option><option>${t('أخصائي تسويق أو اتصال')}</option><option>${t('خدمة عملاء / تجربة عميل')}</option><option>${t('دور آخر')}</option></select></label></div></section><section class="panel consent-panel"><div class="form-section-title"><b>02</b><div><h2>${t('الموافقة والبدء')}</h2><p>${t('يمكنك الانسحاب قبل إرسال الإجابات، والموافقة البحثية مستقلة واختيارية.')}</p></div></div><label class="checkbox-field"><input name="service_consent" type="checkbox" required><span><b>${t('موافقة الخدمة مطلوبة:')}</b> ${t('أوافق على معالجة الإجابات لإصدار التشخيص والتقرير للمنشأة.')}</span></label><label class="checkbox-field"><input name="research_consent" type="checkbox"><span><b>${t('موافقة بحثية اختيارية:')}</b> ${t('أوافق على استخدام البيانات بعد إزالة المعرّفات المباشرة لأغراض البحث والمقارنة.')}</span></label><div class="form-error" hidden></div><button class="primary-button large" type="submit">${t('حفظ البيانات وبدء المقياس')}</button></section></form></main></div>`;
}
...
function impactEffortMatrix(priorities) {
  ...
  if (!points) return `<p class="dashboard-empty">${t('تظهر المصفوفة عند توفر أولويات مرتبطة بالأثر والجهد.')}</p>`;
  return `<div class="impact-matrix" role="img" aria-label="مصفوفة أثر وجهد لأولويات التحسين"><span class="matrix-y high">${t('أثر أعلى')}</span><span class="matrix-y low">${t('أثر أقل')}</span><div class="matrix-plot">${points}</div><div class="matrix-x"><span>${t('جهد أقل')}</span><span>${t('جهد أعلى')}</span></div></div>`;
}
...
function consultationCta(assessmentId) {
  ...
  return `<section class="card consultation-cta" id="consultation-cta">
    <div>
      <span class="dashboard-kicker teal">${t('الخطوة الاختيارية التالية')}</span>
      <h2>${t('هل ترغب في تطوير مستوى النضج الاتصالي في منشأتك؟')}</h2>
      <p>${t('يمكنك طلب جلسة استشارية لمناقشة نتائج منشأتك، وتحديد الأولويات، وبناء خطة عملية لتطوير النضج الاتصالي التسويقي.')}</p>
      <small class="consultation-note">${t('نتيجتك وتقريرها متاحان كاملين سواء طلبت الاستشارة أو لم تطلبها.')}</small>
    </div>
...
async function consultationForm(assessmentId) {
  ...
  openDialog(`<div class="consultation-form-wrap">
    ...
    <p>${t('الحقول المعلّمة مطلوبة. لن تُستخدم بيانات التواصل في احتساب أي درجة أو في المقارنات البحثية.')}</p>
    <form id="consultation-form" class="form-grid">
      <input type="hidden" name="assessment_id" value="${e(String(assessmentId))}">
      <label class="field"><span>${t('الاسم الكامل *')}</span><input name="full_name" required minlength="2" maxlength="160" autocomplete="name"></label>
      <label class="field"><span>${t('البريد الإلكتروني *')}</span><input name="email" type="email" required autocomplete="email"></label>
      <label class="field"><span>${t('رقم الجوال *')}</span><input name="phone_number" required inputmode="tel" autocomplete="tel" placeholder="05xxxxxxxx"></label>
...
async function renderKnowledge() {
  ...
  appView.innerHTML = shell(sections
    ? `${heading}${sections}`
    : `${heading}${emptyState(t('المحتوى المعرفي قيد الإعداد'),t('يكتب الباحث المسؤول عن الأداة هذا المحتوى، ولا يُولَّد آليًا. سيظهر هنا فور اعتماده.'), '<a class="primary-button" href="#methodology">اطلع على المنهجية</a>')}`);
}
...
function renderJoin(route) {
  ...
  appView.innerHTML = `<div class="landing-page">${publicNav()}<main><div class="page narrow-page">
    ${pageHeading(t('مشاركة زملاء العمل'),t('الانضمام بكود مشاركة'),t('زميلك في المنشأة أنشأ كود مشاركة ليجيب أكثر من موظف على نفس التقييم. الإجابات تُجمع في تقييم واحد، ولا يلزمك حساب.'))}
    <section class="panel"><form id="join-code-form" class="form-grid">
      ...
      <label class="field"><span>${t('المسمى الوظيفي')}</span><input name="job_title" maxlength="120"></label>
      <label class="checkbox-field full"><input name="service_consent" type="checkbox" required><span>${t('أوافق على معالجة إجاباتي لإصدار تشخيص المنشأة.')}</span></label>
      <div class="form-error full" hidden></div>
      <button class="primary-button full" type="submit">${t('انضم إلى التقييم')}</button>
    </form></section>
    <section class="panel"><p>${t('لا تملك كودًا؟')} <a href="#participant-start">${t('ابدأ تقييمًا جديدًا لمنشأتك')}</a>.</p></section>
  </div></main></div>`;
}
...
function renderSignup(route) {
  ...
  appView.innerHTML = `<div class="landing-page">${publicNav()}<main><div class="page narrow-page">
    ${pageHeading(t('حساب اختياري'),t('إنشاء حساب مشارك'), claiming
      ? t('احفظ نتيجتك بحساب لتعود إليها لاحقًا، وتتابع خارطة التحسين، وتدعو زملاءك، وتطلب استشارة. نتيجتك متاحة الآن سواء أنشأت حسابًا أو لم تفعل.')
      : t('حساب المشارك اختياري. يتيح لك العودة إلى تقييماتك ونتائجك وخارطة التحسين، ودعوة زملائك بكود مشاركة، وطلب استشارة.'))}
    <section class="panel"><form id="participant-signup-form" class="form-grid">
      ...
      <label class="field full"><span>${t('كلمة المرور')}</span><input name="password" type="password" minlength="10" required autocomplete="new-password"><small>${t('عشرة محارف على الأقل، وتشمل حرفًا كبيرًا وصغيرًا ورقمًا.')}</small></label>
      <div class="form-error full" hidden></div>
      <button class="primary-button full" type="submit">${t('إنشاء الحساب')}</button>
    </form></section>
    <section class="panel"><p>${t('لديك حساب؟')} <a href="#login">${t('تسجيل الدخول')}</a> · <a href="#landing">${t('العودة للرئيسية')}</a></p></section>
  </div></main></div>`;
}
...
async function renderPrivacy() {
  ...
  appView.innerHTML = `<div class="landing-page">${publicNav()}<main><div class="page privacy-page">
    ${pageHeading(t('الشفافية'), notice.title_ar, `الإصدار ${e(notice.version)}`)}
...
function renderPublicResult(result) {
  ...
  appView.innerHTML = `<div class="public-result-page dashboard-result">${publicNav()}<main><section class="result-hero dashboard-hero"><div><span>${t('اكتمل مقياس النضج الاتصالي التسويقي')}</span><h1>${t('تصنيف منشأتك:')} <i>${e(currentStage.label_ar || t('غير متاح'))}</i></h1><p>${e(summary)}</p></div><div class="result-score-cards"><article><small>${t('النضج MCM')}</small><strong>${number(mcmTotal,1)}</strong><span>/100</span></article><article><small>${t('الكفاءة SMCE')}</small><strong>${number(smceTotal,1)}</strong><span>/100</span></article><article><small>${t('الفارق بين الكفاءة والنضج')}</small><strong>${Number(efficiencyGap || 0) > 0 ? '+' : ''}${number(efficiencyGap,1)}</strong><span>${t('نقطة')}</span></article></div></section><section class="panel maturity-dashboard"><div class="card-title"><div><h2>${t('رحلة النضج والخطوة التالية')}</h2><p>${t('المرحلة الحالية مميزة، والحدود تشخيصية أولية قيد التحقق.')}</p></div><div class="next-stage-callout"><small>الفجوة إلى ${e(nextLabel)}</small><strong>${nextGap > 0 ? `${number(nextGap,1)} نقطة` : t('أنت في المرحلة الأعلى')}</strong></div></div><div class="maturity-stages compact">${stages.map(stage=>`<div class="${Number(stage.level_order) === currentOrder ? 'active':''}"><b>${number(stage.level_order)}</b><span>${e(stage.label_ar)}</span></div>`).join('')}</div></section><section class="dashboard-grid"><article class="card dashboard-panel radar-panel"><div class="card-title"><div><span class="dashboard-kicker">${t('بصمة القدرة المؤسسية')}</span><h2>${t('أبعاد النضج السبعة MCM')}</h2><p>${t('كل محور يمثل درجة بُعد مستقلة من 100.')}</p></div></div>${radarChart(mcmDimensions)}</article><article class="card dashboard-panel smce-panel"><div class="card-title"><div><span class="dashboard-kicker teal">${t('النتيجة الاتصالية')}</span><h2>${t('كفاءة التواصل SMCE')}</h2><p>${t('قراءة مستقلة للأداء الاتصالي المرصود.')}</p></div></div>${scoreProfile(smceDimensions,t('لا تتوفر درجات SMCE لهذه الحالة.'),'teal')}<div class="relationship-compact"><b>MCM</b><span>${t('أثر إيجابي مقترح ←')}</span><b>SMCE</b><p>${e(relation.interpretation_ar || '')}</p></div></article></section><section class="dashboard-grid context-outcomes"><article class="card dashboard-panel"><div class="card-title"><div><span class="dashboard-kicker">${t('جاهزية التنفيذ')}</span><h2>${t('الممكنات التنظيمية')}</h2><p>${t('تفسر قدرة المنشأة على التنفيذ ولا تدخل في مقام درجة MCM.')}</p></div></div>${scoreProfile(enablers,t('تظهر الممكنات هنا عند احتساب بنود القيادة والكفاءات والتقنية والبيانات.'),'navy')}</article><article class="card dashboard-panel"><div class="card-title"><div><span class="dashboard-kicker teal">${t('نتائج اختيارية')}</span><h2>${t('الثقة والرضا والعلامة والأعمال')}</h2><p>${t('مؤشرات سياقية اختيارية لا تدخل في التصنيف الأساسي.')}</p></div></div>${scoreProfile(outcomes,t('لم تُجب هذه الحالة عن بنود النتائج الاختيارية، لذلك لم تُعرض درجة.'),'sand')}</article></section><section class="insights-grid"><article class="card"><div class="card-title"><div><span class="dashboard-kicker success">${t('ما يعمل جيدًا')}</span><h2>${t('نقاط القوة')}</h2></div></div><div class="insight-list">${insightCards(strengthItems,'strength',t('لا تتوفر نقاط قوة محسوبة.'))}</div></article><article class="card"><div class="card-title"><div><span class="dashboard-kicker warning">${t('بداية التحسين')}</span><h2>${t('الفرص ذات الأولوية')}</h2></div></div><div class="insight-list">${insightCards(opportunityItems,'opportunity',t('لا تتوفر فرص محسوبة.'))}</div></article></section><section class="dashboard-grid improvement-section"><article class="card dashboard-panel"><div class="card-title"><div><span class="dashboard-kicker">${t('قرار تنفيذي')}</span><h2>${t('مصفوفة الأثر والجهد')}</h2><p>${t('الأرقام تربط النقاط بأولوية التحسين المقابلة.')}</p></div></div>${impactEffortMatrix(priorities.length ? priorities : opportunityItems)}</article><article class="card dashboard-panel priority-panel"><div class="card-title"><div><span class="dashboard-kicker teal">${t('الترتيب المقترح')}</span><h2>${t('أولويات العمل')}</h2><p>${t('ابدأ بالأثر الأعلى والجهد الأقل، ثم راجع الملاءمة مع فريقك.')}</p></div></div><ol>${(priorities.length ? priorities : opportunityItems).slice(0,5).map((item,index)=>`<li><b>${number(item.rank || index+1)}</b><div><h3>${e(item.problem || item.title || dashboardName(item))}</h3>${item.action || item.description ? `<p>${e(item.action || item.description)}</p>` : ''}<span>${e(item.dimension_code || item.code || '')}${item.kpi ? ` · KPI: ${e(item.kpi)}` : ''}</span></div></li>`).join('') || '<li class="dashboard-empty">لا توجد أولويات مادية لهذه النتيجة.</li>'}</ol></article></section><section class="panel roadmap-dashboard"><div class="card-title"><div><span class="dashboard-kicker">${t('من التشخيص إلى التنفيذ')}</span><h2>${t('خارطة التحسين 30 / 90 / 180 يومًا')}</h2><p>${t('خطوات مرحلية قابلة للمراجعة؛ حدّث المسؤول والمؤشر والتاريخ عند اعتماد الخطة داخليًا.')}</p></div></div><div class="roadmap-timeline">${timeline}</div></section>${state.token ? '' : `<section class="card account-cta"><div><span class="dashboard-kicker">${t('اختياري')}</span><h2>${t('تحليل أعمق بحساب مشارك')}</h2><p><strong>${t('نتيجتك أعلاه كاملة، ولا يُحجب منها شيء.')}</strong> ${t('الحساب يفتح تحليلًا إضافيًا لا يظهر في هذه الصفحة:')}</p><ul class="account-benefits"><li>${t('تفاصيل كل بُعد على حدة مع توصيته الخاصة')}</li><li>${t('التشخيص العميق بالأدلة الرقمية ودرجة الثقة')}</li><li>${t('تحليل الفجوات بين الأبعاد المترابطة')}</li><li>${t('المقارنة المرجعية مع منشآت مشابهة')}</li><li>${t('مسار النضج عبر دورات القياس المتتالية')}</li><li>${t('تقرير PDF لتقييمك، ودعوة زملائك بكود مشاركة')}</li></ul></div><div class="button-row"><a class="primary-button" href="#signup">${t('إنشاء حساب')}</a><a class="secondary-button" href="#login">${t('لدي حساب')}</a></div></section>`}${consultationCta(result.assessment_id)}<section class="card result-boundary"><div><h2>${t('كيف تقرأ النتيجة؟')}</h2><p>${e((locale() === 'en' && result.classification_notice_en) ? result.classification_notice_en : (result.classification_notice || ''))}</p></div><div class="button-row public-result-actions"><button class="secondary-button" data-action="print-result">${t('طباعة النتائج')}</button><a class="primary-button" href="#landing">${t('العودة للرئيسية')}</a></div></section></main></div>`;
}
...
function renderAuth() {
  ...
  const ephemeralNotice = state.publicConfig.storage === 'ephemeral-demo' ? `<div class="research-notice"><strong>${t('بيئة عرض مؤقتة')}</strong><span>${e(state.publicConfig.notice || t('لا تستخدم بيانات حقيقية؛ قد يعاد ضبط البيانات.'))}</span></div>` : '';
  ...
  appView.innerHTML = `<section class="auth-layout"><div class="auth-brand"><div class="auth-logo"><span>ن</span>${t('مقياس النضج الاتصالي التسويقي')}</div><div class="auth-copy"><div class="eyebrow">PLATFORM MANAGEMENT</div><h1>${t('قرارات أوضح.')}<br><i>${t('أثر أقوى.')}</i></h1><p>${t('مساحة مدير المنصة لمتابعة الحالات والتقارير وخارطة التطوير وتصدير ملفات التحليل.')}</p><div class="auth-features"><span>${t('تقارير ورسوم تنفيذية')}</span><span>${t('ربط MCM وSMCE')}</span><span>${t('Excel وSPSS')}</span></div></div><small>Evidence-Informed Applied Model</small></div><div class="auth-panel"><div class="auth-box">${ephemeralNotice}<div class="auth-tabs"><button class="${register ? '' : 'active'}" data-action="auth-mode" data-mode="login" type="button">${t('تسجيل الدخول')}</button>${registerTab}</div><h2>${register ? t('أنشئ مساحة مؤسستك') : t('تسجيل الدخول')}</h2><p>${register ? t('ابدأ بملف المؤسسة ثم نفّذ التقييم الأول.') : t('أدخل بيانات الحساب الإداري للوصول إلى مساحة العمل.')}</p><form id="auth-form" class="form-grid">${register ? `<label class="field"><span>${t('الاسم الكامل')}</span><input name="name" required autocomplete="name"></label><label class="field"><span>${t('اسم المؤسسة')}</span><input name="organization_name" required></label>` : ''}<label class="field ${register ? '' : 'full'}"><span>${t('البريد الإلكتروني')}</span><input name="email" type="email" required autocomplete="email"></label><label class="field ${register ? '' : 'full'}"><span>${t('كلمة المرور')}</span><input name="password" type="password" required autocomplete="${register ? 'new-password' : 'current-password'}"></label>${register ? `<label class="checkbox-field full"><input name="service_consent" type="checkbox" required><span>${t('أوافق على معالجة البيانات لأغراض التشخيص وتقديم الخدمة.')}</span></label>` : ''}<div id="auth-error" class="form-error full" hidden></div><button class="primary-button full" type="submit">${register ? t('إنشاء الحساب') : t('تسجيل الدخول')}</button></form>${register ? '' : `<div class="button-row"><button class="secondary-button" data-action="forgot-password" type="button">${t('نسيت كلمة المرور')}</button><a class="secondary-button" href="#participant-start">${t('دخول المشارك مباشرة')}</a><a class="secondary-button" href="#landing">${t('الرئيسية')}</a></div>`}</div></div></section>`;
}

function renderForgot() {
  state.me = null; applyShell();
  appView.innerHTML = `<section class="auth-layout"><div class="auth-brand"><div class="auth-logo"><span>ن</span>${t('مقياس النضج الاتصالي التسويقي')}</div><div class="auth-copy"><h1>${t('استعادة')}<br><i>${t('آمنة للحساب.')}</i></h1><p>${t('لن نكشف ما إذا كان البريد مسجلًا أم لا.')}</p></div></div><div class="auth-panel"><div class="auth-box"><h2>${t('نسيت كلمة المرور')}</h2><p>${t('أدخل بريدك لاستلام تعليمات الاستعادة عبر القناة المضبوطة.')}</p><form id="forgot-form" class="form-grid"><label class="field full"><span>${t('البريد')}</span><input name="email" type="email" required></label><button class="primary-button full">${t('إرسال الطلب')}</button></form><p><a href="#login">${t('العودة لتسجيل الدخول')}</a></p></div></div></section>`;
}
function renderReset(route) {
  state.me = null; applyShell();
  const token = route.query.get('token') || '';
  appView.innerHTML = `<section class="auth-layout"><div class="auth-brand"><div class="auth-logo"><span>ن</span>${t('مقياس النضج الاتصالي التسويقي')}</div><div class="auth-copy"><h1>${t('كلمة مرور')}<br><i>${t('جديدة.')}</i></h1></div></div><div class="auth-panel"><div class="auth-box"><h2>${t('إعادة ضبط كلمة المرور')}</h2><form id="reset-form" class="form-grid"><input type="hidden" name="token" value="${e(token)}"><label class="field full"><span>${t('كلمة المرور الجديدة')}</span><input name="password" type="password" minlength="10" required></label><button class="primary-button full">${t('حفظ كلمة المرور')}</button></form></div></div></section>`;
}

async function renderOverview() {
  loading('جارٍ تحميل لوحة النضج...');
  const data = await api('/api/dashboard');
  const actions = canManageAssessments() ? `<button class="primary-button" data-action="create-assessment" type="button">${t('تقييم جديد')}</button>` : '';
  if (!data.latest) {
    ...
    appView.innerHTML = `<div class="page">${pageHeading(t('مساحة العمل · نظرة عامة'), `مرحبًا، ${state.me.user.name}`, `ابدأ أول قياس لمؤسسة <strong>${e(data.organization.name)}</strong>.`, actions)}${modelNotice()}<div class="card-grid"><article class="card metric-card span-4"><small>${t('اكتمال ملف المؤسسة')}</small><strong>${number(data.profile_completion)}<em>%</em></strong><a href="#settings">${t('عرض الملف')}</a></article><article class="card metric-card span-4"><small>${t('تقييمات قيد التنفيذ')}</small><strong>${number(data.draft_count)}</strong><a href="#assessments">${t('عرض المسودات')}</a></article><article class="card metric-card span-4"><small>${t('النتائج المتاحة')}</small><strong>0</strong><span>${t('تظهر بعد إرسال التقييم')}</span></article></div>${emptyState(t('لم يتم تنفيذ تقييم بعد.'), t('تظهر هنا النتائج بعد اكتمال تقييم يستخدم إصدار الأداة الحالي.'), emptyAction)}</div>`;
    return;
  }
  ...
  const diagnoses = data.diagnoses.length ? data.diagnoses.map(item => `<article class="card diagnosis-card ${item.severity === 'HIGH' ? 'high' : ''}">${badge(item.severity, item.severity === 'HIGH' ? 'danger' : 'warning')}<h3>${e(item.name)}</h3><p>${e(item.business_implication)}</p><a href="#diagnosis/${result.assessment_id}">${t('عرض الأدلة')}</a></article>`).join('') : `<article class="card"><h3>${t('لا توجد قواعد تشخيص مفعلة على النتيجة')}</h3><p>${t('لا يعني ذلك اعتماد الأداة؛ راجع الأبعاد والأولويات المتاحة.')}</p></article>`;
  ...
  appView.innerHTML = `<div class="page">${pageHeading(`آخر تقييم · ${dateText(result.completed_at)}`, `صباح الخير، ${state.me.user.name}`, `هذه قراءة آخر تقييم مكتمل لمؤسسة <strong>${e(data.organization.name)}</strong> · مصدر البيانات ${e(result.data_origin)}.`, actions)}${modelNotice()}<div class="card-grid"><article class="card metric-card teal span-4"><small>${t('درجة MCM الكلية')}</small><strong>${number(mcm.total,1)}<em>/100</em></strong><span>${e(mcm.maturity_level?.label_ar || t('تصنيف غير متاح'))}</span></article><article class="card metric-card span-4"><small>${t('كفاءة التواصل SMCE')}</small><strong>${number(smce.total,1)}<em>/100</em></strong><span>${t('نتيجة مستقلة عن MCM')}</span></article><article class="card metric-card span-4"><small>${t('اكتمال ملف المؤسسة')}</small><strong>${number(data.profile_completion)}<em>%</em></strong><a href="#settings">${t('عرض الملف')}</a></article><article class="card span-8"><div class="card-title"><div><h2>${t('أبعاد النضج الاتصالي')}</h2><p>${t('الأبعاد السبعة مستقلة وقابلة للتشخيص.')}</p></div><a href="#results/${result.assessment_id}">${t('كل النتائج')}</a></div><div class="dimension-list">${dimensions}</div></article><article class="card span-4"><div class="card-title"><div><h2>${t('قراءة تشخيصية')}</h2><p>${t('قواعد حتمية لا تعتمد على الذكاء الاصطناعي.')}</p></div></div>${diagnoses}</article><article class="card span-12"><div class="card-title"><div><h2>${t('مسار النضج')}</h2><p>${t('التقييمات المكتملة فقط.')}</p></div><a href="#history">${t('التاريخ الكامل')}</a></div><div class="trend-chart">${history}</div></article></div><section class="panel"><div class="card-title"><div><h2>${t('أولويات التطوير')}</h2><p>${t('ترتيب قابل للتدقيق من الفجوة والأثر والاعتمادية والجهد.')}</p></div><a href="#roadmap/${result.assessment_id}">${t('عرض الخارطة')}</a></div>${data.priorities.length ? table([t('الترتيب'),t('البعد'),t('الحالي'),t('الفجوة'),t('المالك'),''], priorityRows, 6, 820) : emptyState(t('لا توجد أولويات بعد'), t('لم تُولد أولويات لهذه النتيجة.'))}</section></div>`;
}
...
async function renderAssessments() {
  ...
  const rows = data.assessments.map(item => `<div class="data-table__row"><strong>#${item.id}<small>${e(item.assessment_type)}</small></strong><span>${badge(item.status, item.status === 'COMPLETED' ? '' : 'warning')}</span><span>${e(item.instrument_version)} · ${e(item.instrument_status)}</span><span>${dateText(item.created_at)}</span><span>${number(item.mcm_total,1)}</span><span class="button-row">${item.status === 'COMPLETED' ? `<a class="secondary-button" href="#results/${item.id}">${t('النتائج')}</a>${canManageAssessments() ? `<button class="secondary-button" data-action="repeat-assessment" data-id="${item.id}">${t('إعادة')}</button>` : ''}` : `<a class="primary-button" href="#assessment/${item.id}">${t('فتح')}</a>`}${item.status !== 'COMPLETED' ? `<button class="secondary-button" data-action="share-join-code" data-id="${item.id}">${t('دعوة زميل')}</button>` : ''}${hasRole('SUPER_ADMIN') ? `<button class="danger-button" data-action="delete-assessment" data-id="${item.id}">${t('حذف')}</button>` : ''}</span></div>`);
  appView.innerHTML = `<div class="page">${pageHeading(t('مساحة العمل · التقييمات'),t('التقييمات المؤسسية'),t('الحفظ والاستكمال مرتبطان بالمشارك وصلاحيته داخل المؤسسة.'), createAction)}${modelNotice()}<section class="panel">${rows.length ? table([t('التقييم'),t('الحالة'),t('إصدار الأداة'),t('تاريخ البدء'),'MCM',t('الإجراء')], rows, 6, 850) : emptyState(t('لا توجد تقييمات'), t('لا توجد تقييمات معيّنة لهذا الحساب.'), createAction)}</section></div>`;
}
...
function questionCard(item, response, disabled = false) {
  ...
  let control;
  if (type === 'TEXT') {
    control = `<label class="field answer-field"><span>${t('الإجابة النصية')}</span><textarea name="q_${item.id}" data-answer-input data-item-id="${item.id}" rows="4"${lock}>${response?.missing_type ? '' : e(value)}</textarea></label>`;
...
async function renderResults(route) {
  ...
  if (!id) {
    ...
    if (!id) {
      ...
      appView.innerHTML = `<div class="page">${pageHeading(t('مساحة العمل · النتائج'),t('النتائج'),t('تظهر النتائج بعد إرسال تقييم مكتمل.'))}${
        pending
          ? emptyState(t('لديك تقييم غير مكتمل.'), t('أكمل الإجابة على البنود المطلوبة ثم أرسل التقييم لعرض النتائج.'), `<a class="primary-button" href="#assessment/${pending}">${t('استكمال التقييم')}</a>`)
          : emptyState(t('لم تكمل أي تقييم حتى الآن.'), t('ابدأ التقييم لتحصل على تصنيف منشأتك وخارطة التحسين.'), '<a class="primary-button" href="#assessments">ابدأ التقييم</a>')
      }</div>`;
      return;
    }
  }
  ...
  const context = data.context ? `<section class="panel"><div class="card-title"><div><h2>${t('السياق الديموغرافي للحالة')}</h2><p>${t('عوامل تفسيرية لا تدخل في مقام درجة MCM.')}</p></div></div><div class="pill-list"><span>القطاع: ${e(data.context.sector)}</span><span>الحجم: ${e(data.context.firm_size)}</span><span>العمر: ${number(data.context.firm_age_years)} سنة</span><span>المنطقة: ${e(data.context.region)}</span><span>منصات التواصل: ${number(data.context.social_platform_count)}</span><span>دور المجيب: ${e(data.context.respondent_role)}</span></div></section>` : '';
  const actions = `<a class="secondary-button" href="#diagnosis/${id}">${t('التشخيص')}</a><a class="secondary-button" href="#gaps/${id}">${t('الفجوات')}</a><a class="secondary-button" href="#priorities/${id}">${t('الأولويات')}</a><a class="primary-button" href="#roadmap/${id}">${t('خارطة التحسين')}</a>`;
  const reportAction = canManageAssessments() ? `<button class="primary-button" data-action="generate-report" data-id="${id}" data-type="EXECUTIVE">${t('5. إنشاء التقرير')}</button>` : '';
  const aiPlanAction = canManageAssessments() ? `<button class="secondary-button" data-action="generate-ai-plan" data-id="${id}">${t('6. تحليل خطة التحسين')}</button>` : '';
  appView.innerHTML = `<div class="page">${pageHeading(`نتائج التقييم #${id}`,`نتيجة ${e(data.organization_name)}`,`اكتمل القياس بتاريخ ${e(dateText(data.completed_at))}.`, actions)}${modelNotice()}<section class="panel maturity-panel"><div class="card-title"><div><h2>تصنيف المنشأة: ${e(mcm.maturity_level?.label_ar || t('غير مصنف'))}</h2><p>${t('مرحلة تشخيصية أولية ضمن مسار من خمس مراحل.')}</p></div><strong class="level-score">${number(mcm.total,1)} / 100</strong></div><div class="maturity-stages compact">${stages}</div></section><div class="card-grid"><article class="card span-6"><div class="card-title"><div><h2>${t('النضج الاتصالي MCM')}</h2><p>${t('متوسط موزون للأبعاد السبعة فقط.')}</p></div>${badge(mcm.maturity_level?.label_ar || t('غير مصنف'))}</div><div class="score-layout"><div class="score-ring" style="--score:${Number(mcm.total || 0)}"><div><strong>${number(mcm.total,1)}</strong><small>/100</small></div></div><div class="dimension-list">${dimLines}</div></div></article><article class="card span-6"><div class="card-title"><div><h2>${t('كفاءة التواصل SMCE')}</h2><p>${t('نتيجة اتصالية لاحقة مقترحة، وتُحتسب بصورة مستقلة.')}</p></div></div><div class="score-layout"><div class="score-ring" style="--score:${Number(smce.total || 0)}"><div><strong>${number(smce.total,1)}</strong><small>/100</small></div></div><div class="dimension-list">${smceLines}</div></div></article><article class="card span-12 relationship-card"><div><small>${t('النموذج المقترح')}</small><h2>MCM <span>←</span> SMCE</h2><p>${e(relation.interpretation_ar || '')}</p></div><div class="relationship-gap"><small>${t('الكفاءة ناقص النضج')}</small><strong>${Number(relation.efficiency_minus_maturity || 0) > 0 ? '+' : ''}${number(relation.efficiency_minus_maturity,1)}</strong><span>${e(relation.narrative_ar || '')}</span></div></article></div>${context}<section class="panel"><div class="card-title"><div><h2>${t('من النتيجة إلى التنفيذ')}</h2><p>${t('انتقل بالتسلسل من الأدلة إلى الفجوة والأولوية ثم خارطة العمل.')}</p></div></div><div class="button-row"><a class="secondary-button" href="#diagnosis/${id}">${t('1. التشخيص العميق')}</a><a class="secondary-button" href="#gaps/${id}">${t('2. تحليل الفجوات')}</a><a class="secondary-button" href="#priorities/${id}">${t('3. ترتيب الأولويات')}</a><a class="secondary-button" href="#benchmark/${id}">${t('4. المقارنة المرجعية')}</a>${reportAction}${aiPlanAction}</div><p class="classification-boundary">${e(data.classification_notice)}</p></section>${consultationCta(id)}</div>`;
}
...
async function renderDimension(route) {
  ...
  appView.innerHTML = `<div class="page">${pageHeading(`نتائج #${id} · ${e(code)}`,e(item.name || code),e(item.name_en || ''),`<a class="secondary-button" href="#results/${id}">${t('العودة للنتائج')}</a>`)}${modelNotice()}<div class="card-grid"><article class="card metric-card teal span-4"><small>${t('درجة البعد')}</small><strong>${number(item.score,1)}<em>/100</em></strong><span>${number(item.answered_count)} من ${number(item.eligible_count)} بندًا</span></article><article class="card span-8"><h2>${t('التفسير والإجراء')}</h2>${data.recommendation ? `<h3>${e(data.recommendation.problem)}</h3><p>${e(data.recommendation.action)}</p><div class="pill-list"><span>المالك: ${e(data.recommendation.suggested_owner)}</span><span>الأثر: ${e(data.recommendation.expected_impact)}</span><span>الجهد: ${e(data.recommendation.effort)}</span><span>KPI: ${e(data.recommendation.kpi)}</span></div>` : '<p>لا توجد توصية مرتبطة بهذا الإصدار.</p>'}</article></div><section><div class="card-title"><div><h2>${t('التشخيصات المرتبطة')}</h2><p>${e(data.evidence_notice)}</p></div></div><div class="card-grid">${diagnoses || emptyState(t('لا توجد قاعدة مطابقة'), t('لم تتحقق شروط تشخيص مرتبطة بهذا البعد.'))}</div></section></div>`;
}
...
async function renderDiagnosis(route) {
  ...
  const cards = data.diagnoses.map(item => `<article class="card diagnosis-card ${item.severity === 'HIGH' ? 'high' : ''}"><div class="card-title"><div>${badge(item.severity,item.severity === 'HIGH' ? 'danger':'warning')}<h3>${e(item.name)}</h3></div><strong>${number(item.confidence * 100)}%</strong></div><div class="evidence-list">${Object.entries(item.evidence).map(([key,value]) => `<span>${e(key)}: ${number(value,1)}</span>`).join('')}</div><p><strong>${t('التفسير:')}</strong> ${e(item.interpretation)}</p><p><strong>${t('الأثر:')}</strong> ${e(item.business_implication)}</p></article>`).join('');
  appView.innerHTML = `<div class="page">${pageHeading(`التقييم #${id}`,t('التشخيص العميق'),t('قواعد بيانات حتمية مع أدلة ظاهرة وثقة مسجلة.'),`<a class="secondary-button" href="#results/${id}">${t('النتائج')}</a><a class="primary-button" href="#priorities/${id}">${t('الأولويات')}</a>`)}${modelNotice()}<div class="card-grid">${cards || emptyState(t('لا توجد تشخيصات مطابقة'), t('لم تتحقق الشروط الرقمية للقواعد النشطة في هذا الإصدار.'))}</div></div>`;
}
...
async function renderGaps(route) {
  ...
  appView.innerHTML = `<div class="page">${pageHeading(`التقييم #${id}`,t('تحليل الفجوات'),`تظهر الفجوة عندما يتجاوز الفرق الحد المضبوط (${number(data.threshold,1)} نقطة).`,`<a class="secondary-button" href="#diagnosis/${id}">${t('التشخيص')}</a><a class="primary-button" href="#priorities/${id}">${t('ترتيب الأولويات')}</a>`)}<section class="panel">${rows.length ? table([t('النمط'),t('المصدر'),t('الهدف'),t('الفجوة'),t('الحد')],rows,5,760) : emptyState(t('لا توجد فجوات عابرة للأبعاد'), t('لم يتجاوز أي نمط حد الفجوة المضبوط.'))}</section></div>`;
}
...
async function renderPriorities(route) {
  ...
  appView.innerHTML = `<div class="page">${pageHeading(`التقييم #${id}`,t('أولويات التطوير'),t('ترتيب حتمي قابل للتدقيق، ولا تدخل الفجوات السالبة في الأولوية.'),`<a class="secondary-button" href="#gaps/${id}">${t('الفجوات')}</a><a class="primary-button" href="#roadmap/${id}">${t('تحويلها إلى خطة')}</a>`)}<div class="card-grid">${cards || emptyState(t('لا توجد أولويات'), t('لم تُولد توصيات قابلة للترتيب لهذا الإصدار.'))}</div></div>`;
}
...
async function renderRoadmap(route) {
  ...
  if (!id) { appView.innerHTML = `<div class="page">${pageHeading(t('مساحة العمل'),t('خارطة التحسين'),t('تُنشأ الخارطة بعد اكتمال تقييم.'),'<a class="primary-button" href="#assessments">اذهب للتقييمات</a>')}${emptyState(t('لا توجد خارطة بعد'),t('أكمل أول تقييم ليحوّل المحرك الأولويات إلى خطة 30/90/180 يومًا.'))}</div>`; return; }
  loading(); const data = await api(`/api/roadmap/${id}`);
  const columns = [['0-30','0–30 يومًا'],['31-90','31–90 يومًا'],['3-6','3–6 أشهر']].map(([key,label]) => `<section class="roadmap-column"><h2>${label}</h2>${(data.roadmap[key] || []).map(item => `<article class="roadmap-item"><h3>${e(item.title)}</h3><p>${e(item.description)}</p><div class="roadmap-meta"><span>${e(item.owner || t('غير معيّن'))}</span><span>${e(item.target_date || '--')}</span>${badge(item.status,item.status === 'COMPLETED' ? '' : 'neutral')}</div>${['COMPANY_ADMIN','SUPER_ADMIN'].includes(state.me.user.role) ? `<button class="secondary-button" data-action="edit-roadmap" data-assessment-id="${id}" data-id="${item.id}" data-owner="${e(item.owner || '')}" data-date="${e(item.target_date || '')}" data-status="${e(item.status)}" type="button">${t('تحديث')}</button>` : ''}</article>`).join('') || '<p>لا توجد إجراءات في هذا الأفق.</p>'}</section>`).join('');
  const reportAction = canManageAssessments() ? `<button class="primary-button" data-action="generate-report" data-id="${id}" data-type="DETAILED">${t('تقرير تفصيلي')}</button>` : '';
  appView.innerHTML = `<div class="page">${pageHeading(`التقييم #${id}`,t('خارطة التحسين'),t('حوّل التشخيص إلى مالك وتاريخ وحالة ومؤشر قياس.'),`<a class="secondary-button" href="#priorities/${id}">${t('الأولويات')}</a>${reportAction}`)}<div class="roadmap-board">${columns}</div></div>`;
}
...
async function renderHistory() {
  ...
  appView.innerHTML = `<div class="page">${pageHeading(t('مساحة العمل · التاريخ'),t('مسار النضج'),t('كل نقطة مرتبطة بتاريخ تقييم فعلي وإصدار أداة محفوظ.'))}<section class="card"><div class="trend-chart">${trend || '<p>لا توجد نقاط بعد.</p>'}</div></section><section class="panel">${rows.length ? table([t('التقييم'),t('التاريخ'),t('الإصدار'),'MCM',t('المستوى'),'SMCE',''],rows,7,900) : emptyState(t('لا يوجد تاريخ بعد'),t('أكمل تقييمين على الأقل لمقارنة المسار.'))}</section></div>`;
}
...
async function renderBenchmark(route) {
  ...
  const filters = `<div class="button-row"><a class="secondary-button" href="#benchmark/${id}?cohort=overall">${t('الإجمالي')}</a><a class="secondary-button" href="#benchmark/${id}?cohort=sector">${t('القطاع')}</a><a class="secondary-button" href="#benchmark/${id}?cohort=size">${t('الحجم')}</a></div>`;
  const content = data.available ? `<div class="card-grid"><article class="card metric-card span-4"><small>${t('نتيجة المؤسسة')}</small><strong>${number(data.company_score,1)}</strong></article><article class="card metric-card span-4"><small>${t('متوسط المجموعة')}</small><strong>${number(data.cohort_average,1)}</strong></article><article class="card metric-card span-4"><small>${t('الربع الأعلى')}</small><strong>${number(data.top_quartile,1)}</strong></article></div><p>حجم العينة: ${number(data.sample_size)} · وحدة التحليل: آخر تقييم مكتمل لكل مؤسسة.</p>` : emptyState('لا تتوفر عينة كافية حاليًا لإظهار مقارنة موثوقة.', data.reason === 'NON_REAL_ASSESSMENT' ? 'التقييمات التجريبية لا تدخل في المقارنة الواقعية.' : `تُحجب المقارنة عندما تكون العينة أقل من ${number(data.minimum_sample)}، دون كشف حجم الخلية الدقيق.`);
  appView.innerHTML = `<div class="page">${pageHeading(`التقييم #${id}`,t('المقارنة المرجعية'),t('تطبيق صارم للموافقة، مصدر البيانات، الإصدار، والحد الأدنى للعينة.'),filters)}${content}</div>`;
}
...
async function renderReports() {
  ...
  appView.innerHTML = `<div class="page">${pageHeading(t('مساحة العمل · التقارير'),t('التقارير التنفيذية'),t('تقارير PDF حقيقية مرتبطة بنسخة التقييم والأداة.'))}${modelNotice()}<section class="panel"><div class="card-title"><div><h2>${t('تقرير جديد')}</h2><p>${t('اختر تقييمًا مكتملًا.')}</p></div></div>${form || reportEmpty}</section><section class="panel">${rows.length ? table([t('التقرير'),t('النوع'),t('الحالة'),t('الإنشاء'),''],rows,5,720) : emptyState(t('لا توجد تقارير'),t('لم تُنشأ تقارير متاحة بعد.'))}</section></div>`;
}
...
async function renderParticipants() {
  ...
  const rows = participants.participants.map(item => `<div class="data-table__row"><strong>${e(item.full_name || t('مشارك مباشر'))}<small dir="ltr">${e(item.research_id || '')}</small></strong><span>${item.assessment_id ? `#${number(item.assessment_id)}` : '--'}</span><span>${e(item.job_title || '--')}</span><span>${e(item.department || '--')}</span><span>${badge(item.assessment_status || item.participation_status, item.assessment_status === 'COMPLETED' ? '' : 'warning')}</span><span>${dateText(item.completed_at || item.created_at)}</span><span class="button-row">${item.assessment_status === 'COMPLETED' && item.assessment_id ? `<a class="secondary-button" href="#results/${item.assessment_id}">${t('النتيجة')}</a>` : ''}</span></div>`);
  const startAction = `<a class="primary-button" href="#participant-start">${t('ابدأ تقييمًا مباشرًا')}</a>`;
  appView.innerHTML = `<div class="page">${pageHeading(t('مساحة العمل · المشاركون'),t('المشاركون'),t('الدخول مباشر بلا دعوات وبلا حسابات؛ تظهر هنا الحالات التي بدأت فعلًا وحالة كل منها.'), startAction)}<section class="panel"><div class="card-title"><div><h2>${t('تقييمات نشطة')}</h2><p>${active.length ? `${number(active.length)} تقييم قيد التنفيذ في هذه المؤسسة.` : t('لا توجد تقييمات قيد التنفيذ حاليًا.')}</p></div></div>${rows.length ? table([t('المشارك'),t('التقييم'),t('المسمى'),t('القسم'),t('الحالة'),t('التاريخ'),''],rows,7,940) : emptyState(t('لا يوجد مشاركون بعد'),t('يبدأ المشارك المقياس مباشرة من الصفحة العامة دون دعوة أو حساب.'),startAction)}</section></div>`;
}
...
async function renderParticipantAssessment(route) {
  ...
  if (data.assessment.status === 'COMPLETED' || data.assessment.participant_status === 'COMPLETED') {
    ...
    else { sessionStorage.removeItem('mcm_participant_token'); appView.innerHTML = `<section class="route-state">${emptyState(t('شكرًا لمشاركتك'),t('إجاباتك مرسلة ومقفلة، ولا يلزم إجراء إضافي.'),'<a class="primary-button" href="#landing">إنهاء</a>')}</section>`; }
    return;
  }
  ...
  appView.innerHTML = `<div class="page assessment-page participant-measure">${pageHeading(`تقييم #${data.assessment.id}`,t('مقياس النضج الاتصالي التسويقي'),`أجب عن ${number(visibleItems.length)} عبارة وفق واقع المنشأة خلال الاثني عشر شهرًا الماضية. استخدم «لا ينطبق» أو «لا أعرف» فقط عند الحاجة.`)} ${modelNotice()}<section class="assessment-header"><div class="progress-row"><strong>${t('التقدم')} <span id="progress-value">${number(data.review.progress)}%</span></strong><span class="save-state" id="save-state">${t('محفوظ تلقائيًا')}</span></div><div class="progress-track"><span id="progress-bar" style="--progress:${data.review.progress}%"></span></div></section><form id="assessment-form">${sections}</form><section class="panel submit-panel"><p>${t('راجع البنود المطلوبة قبل الإرسال. بعد الإرسال ستُقفل الإجابات وتظهر النتائج وخارطة التحسين.')}</p><button class="primary-button large" data-action="submit-participant" type="button">${t('إرسال التقييم وعرض النتائج')}</button></section></div>`;
}

async function renderNotifications() {
  loading(); const data = await api('/api/notifications');
  const cards = data.notifications.map(item => `<article class="card"><div class="card-title"><div>${badge(item.read_at ? t('مقروء') : t('جديد'),item.read_at ? 'neutral':'')}<h3>${e(item.title)}</h3></div><small>${dateText(item.created_at)}</small></div><p>${e(item.body)}</p><div class="button-row">${item.target_url ? `<a class="secondary-button" href="${e(item.target_url)}">${t('فتح')}</a>` : ''}${!item.read_at ? `<button class="secondary-button" data-action="read-notification" data-id="${item.id}">${t('تحديد كمقروء')}</button>` : ''}</div></article>`).join('');
  appView.innerHTML = `<div class="page">${pageHeading(t('مساحة العمل · التنبيهات'),t('الإشعارات'),t('أحداث التقييمات والدعوات وإصدارات الأداة.'), '<button class="secondary-button" data-action="read-all-notifications">تحديد الكل كمقروء</button>')}<div class="card-grid">${cards || emptyState(t('لا توجد إشعارات'),t('ستظهر الأحداث المرتبطة بمساحة العمل هنا.'))}</div></div>`;
}
...
async function renderSettings() {
  ...
  const companyContent = canManageCompany() ? `<form id="company-profile-form" class="form-grid"><label class="field full"><span>${t('اسم المؤسسة')}</span><input name="name" value="${e(profile.organization.name)}" required></label><label class="field"><span>${t('القطاع')}</span><input name="sector" value="${e(profile.organization.sector)}"></label><label class="field"><span>${t('الحجم')}</span><select name="size"><option value="">${t('غير محدد')}</option>${['MICRO','SMALL','MEDIUM','LARGE'].map(value => `<option value="${value}" ${profile.organization.size === value ? 'selected':''}>${value}</option>`).join('')}</select></label><label class="field"><span>${t('الموقع')}</span><input name="website" type="url" value="${e(profile.profile?.website)}"></label><label class="field"><span>${t('نموذج الأعمال')}</span><input name="business_model" value="${e(profile.profile?.business_model)}"></label><label class="field"><span>${t('عدد الموظفين')}</span><input name="employee_count" value="${e(profile.profile?.employee_count)}"></label><label class="field"><span>${t('حجم فريق الاتصال')}</span><input name="communication_team_size" value="${e(profile.profile?.communication_team_size)}"></label><label class="field"><span>CRM</span><input name="crm_system" value="${e(profile.profile?.crm_system)}"></label><label class="field"><span>${t('نظام التحليلات')}</span><input name="analytics_system" value="${e(profile.profile?.analytics_system)}"></label><label class="checkbox-field full"><input name="research_consent" type="checkbox" ${researchConsent ? 'checked':''}><span>${t('موافقة المؤسسة على إدراج الحالات الواقعية المؤهلة بصورة مجهولة.')}</span></label><button class="primary-button full">${t('حفظ ملف المؤسسة')}</button></form>` : `<div class="pill-list"><span>${e(profile.organization.name)}</span><span>${e(profile.organization.sector || t('قطاع غير محدد'))}</span><span>${e(profile.organization.size || t('حجم غير محدد'))}</span><span>موافقة المؤسسة البحثية: ${researchConsent ? t('مفعلة') : t('غير مفعلة')}</span></div><p>${t('تعديل ملف المؤسسة وموافقتها مخصص لمدير الشركة.')}</p>`;
  appView.innerHTML = `<div class="page">${pageHeading(t('مساحة العمل · الإعدادات'),t('الإعدادات والخصوصية'),t('إدارة الحساب والمؤسسة والموافقات من مكان واحد.'))}<div class="card-grid"><section class="card span-6"><div class="card-title"><div><h2>${t('الملف الشخصي')}</h2><p>${t('بيانات الحساب والواجهة العربية.')}</p></div></div><form id="settings-form" class="form-grid"><input type="hidden" name="locale" value="ar"><label class="field full"><span>${t('الاسم')}</span><input name="name" value="${e(settings.user.name)}" required></label><label class="field"><span>${t('المسمى')}</span><input name="job_title" value="${e(settings.user.job_title)}"></label><label class="field"><span>${t('الجوال')}</span><input name="phone" value="${e(settings.user.phone)}"></label><label class="checkbox-field"><input name="email_notifications" type="checkbox" ${settings.settings?.email_notifications ? 'checked':''}><span>${t('إشعارات البريد')}</span></label><button class="primary-button full">${t('حفظ الحساب')}</button></form></section><section class="card span-6"><div class="card-title"><div><h2>${t('ملف المؤسسة')}</h2><p>الاكتمال الحالي ${number(profile.profile?.completion)}%.</p></div></div>${companyContent}</section><section class="card span-12"><div class="card-title"><div><h2>${t('الفريق والصلاحيات')}</h2><p>${t('أعضاء المؤسسة الحاليون.')}</p></div></div><div class="pill-list">${team.members.map(item => `<span>${e(item.name)} · ${e(roleLabels[item.role] || item.role)} · ${e(item.status)}</span>`).join('')}</div></section><section class="card span-12"><h2>${t('الموافقات المنفصلة')}</h2><p>${t('موافقتك الشخصية على البحث مستقلة عن موافقة المؤسسة. لا تدخل الحالة الواقعية في البحث إلا باجتماع الموافقات المطلوبة.')}</p><form id="personal-consent-form" class="form-grid"><label class="checkbox-field full"><input name="accepted" type="checkbox" ${personalResearch?.accepted ? 'checked':''}><span>${t('أوافق اختياريًا على استخدام إجاباتي المجهولة لأغراض البحث والمقارنة.')}</span></label><button class="secondary-button full">${t('حفظ موافقتي البحثية')}</button></form><div class="pill-list">${consents.consents.map(item => `<span>${e(item.consent_type)} · ${item.accepted ? t('مقبولة'):t('مرفوضة')} · v${e(item.consent_version)}</span>`).join('')}</div></section></div></div>`;
}
...
async function renderMethodology() {
  ...
  appView.innerHTML = `<div class="page">${pageHeading(t('المعرفة · المنهجية'),t('كيف يعمل مقياس النضج الاتصالي التسويقي'),t('فصل واضح بين السياق، القدرة المؤسسية، والكفاءة الاتصالية الناتجة.'))}${modelNotice()}<div class="card-grid"><article class="card span-4"><span class="status-badge">1</span><h2>${t('السياق والعوامل التمكينية')}</h2><p>${t('دعم القيادة والكفاءات البشرية والبنية التقنية وجاهزية البيانات، إضافة إلى حجم المنشأة وعمرها وقطاعها وعدد المنصات. تفسر الظروف ولا تدخل في درجة MCM.')}</p></article><article class="card span-4"><span class="status-badge">2</span><h2>${t('MCM · القدرة المؤسسية')}</h2><p>${t('سبعة أبعاد تُطبّع بنودها إلى 0–100 ثم تجمع بأوزان إصدار الأداة.')}</p></article><article class="card span-4"><span class="status-badge">3</span><h2>${t('SMCE · النتيجة الاتصالية')}</h2><p>${t('خمسة أبعاد تُحتسب بصورة مستقلة لاختبار الأثر الإيجابي المقترح للنضج على الكفاءة.')}</p></article><article class="card span-6"><h2>${t('أبعاد MCM')}</h2><div class="pill-list">${mcm.map(item => `<span>${e(item)}</span>`).join('')}</div></article><article class="card span-6"><h2>${t('أبعاد SMCE')}</h2><div class="pill-list">${smce.map(item => `<span>${e(item)}</span>`).join('')}</div></article><article class="card span-12"><h2>${e(model.label_ar || '')}</h2>${((locale() === 'en' && model.methodology_en) ? model.methodology_en : (model.methodology_ar || [])).map(paragraph=>`<p>${e(paragraph)}</p>`).join('')}<p class="classification-boundary">${e(model.result_disclaimer_ar || '')}</p></article><article class="card span-12"><h2>${t('المراحل الخمس')}</h2><ol class="landing-maturity methodology-stages">${stageCards(maturity.levels || [])}</ol></article><article class="card span-12"><h2>${t('عقد علمي واضح')}</h2><ul><li>${t('الإجابة المفقودة لا تتحول إلى صفر.')}</li><li>${t('الاحتساب والتصنيف والتشخيص تجري على الخادم.')}</li><li>${t('كل نتيجة ترتبط بإصدار أداة وإصدار احتساب.')}</li><li>${t('العلاقة MCM ← SMCE مقترحة وليست ادعاءً سببيًا مثبتًا.')}</li><li>${t('تخضع الأوزان وحدود المستويات ومكتبة التوصيات للمراجعة والتحسين المستمر مع تراكم البيانات.')}</li></ul></article></div></div>`;
}
...
async function renderResearch() {
  ...
  appView.innerHTML = `<div class="page">${pageHeading(t('البحث والتحليل'),t('لوحة الباحث'),t('إحصاءات فعلية فقط، ولا تُعرض أرقام مصطنعة عند غياب الملاحظات.'),`<a class="secondary-button" href="#dataset">${t('مجموعة البيانات')}</a><a class="primary-button" href="#exports">${t('مركز التصدير')}</a>`)}${modelNotice()}<div class="card-grid"><article class="card metric-card span-3"><small>${t('إجمالي الحالات')}</small><strong>${number(data.total_cases)}</strong></article><article class="card metric-card span-3"><small>${t('مكتملة')}</small><strong>${number(data.completed)}</strong></article><article class="card metric-card span-3"><small>${t('غير مكتملة')}</small><strong>${number(data.incomplete)}</strong></article><article class="card metric-card span-3"><small>${t('موافقة بحثية')}</small><strong>${number(data.research_consent_organizations)}</strong></article><article class="card span-6"><h2>${t('مصدر البيانات')}</h2><div class="pill-list">${Object.entries(data.origins).map(([key,value]) => `<span>${e(key)}: ${number(value)}</span>`).join('') || '<span>لا توجد بيانات</span>'}</div></article><article class="card span-6"><h2>${t('إصدار الأداة الحالي')}</h2><p>${data.current_instrument ? `${e(data.current_instrument.version)} · ${e(data.current_instrument.status)}` : t('لا يوجد إصدار.')}</p><a href="#instruments">${t('إدارة الإصدارات')}</a></article></div><section class="panel"><div class="card-title"><div><h2>${t('الحالات حسب القطاع')}</h2><p>${t('عد فعلي من قاعدة البيانات.')}</p></div></div>${sectorRows.length ? table([t('القطاع'),t('الحالات')],sectorRows,2,420) : emptyState(t('لا توجد بيانات كافية حتى الآن.'),t('تظهر التوزيعات بعد وصول حالات فعلية أو تجريبية وفق الفلتر.'))}</section></div>`;
}
...
async function renderDataset(route) {
  ...
  const filters = `<div class="button-row"><a class="secondary-button" href="#dataset?origin=REAL">${t('واقعية فقط')}</a><a class="secondary-button" href="#dataset?origin=SYNTHETIC">${t('اصطناعية')}</a><a class="secondary-button" href="#dataset?origin=DEMO_TEST">Demo/Test</a></div>`;
  appView.innerHTML = `<div class="page">${pageHeading(t('البحث · البيانات'),t('مستكشف مجموعة البيانات'),`المعرّفات مجهولة وPII غير مدرج افتراضيًا. عدد الصفوف: ${number(data.row_count)}.`,filters)}<section class="panel">${rows.length ? table(columns,rows,columns.length,Math.max(900,columns.length*150)) : emptyState(t('لا توجد بيانات بحثية متاحة حتى الآن.'),t('الفلتر الافتراضي REAL ONLY ويتطلب موافقة بحثية صريحة.'))}</section></div>`;
}

async function renderInstruments() {
  loading(); const data = await api('/api/instruments');
  const rows = data.versions.map(item => `<div class="data-table__row"><strong>${e(item.version)}<small>${e(item.name)}</small></strong><span>${badge(item.product_status, item.product_status === 'ACTIVE' ? '' : 'warning')}</span><span>${badge(item.scientific_status, item.scientific_status === 'EMPIRICALLY_VALIDATED' ? '' : 'warning')}</span><span>${number(item.item_count)}</span><span>${number(item.assessment_count)}</span><span class="button-row"><a class="secondary-button" href="#instrument/${item.id}">${t('فتح')}</a>${item.product_status === 'DRAFT' ? `<button class="primary-button" data-action="publish-instrument" data-id="${item.id}">${t('تفعيل الإصدار')}</button>` : `<button class="secondary-button" data-action="duplicate-instrument" data-id="${item.id}">${t('نسخ لمسودة')}</button>`}<button class="secondary-button" data-action="scientific-status" data-id="${item.id}" data-current="${e(item.scientific_status)}">${t('الحالة العلمية')}</button></span></div>`);
  appView.innerHTML = `<div class="page">${pageHeading(t('البحث · الأداة'),t('إصدارات الأداة'),t('النسخ المستخدمة غير قابلة للتعديل؛ أي تغيير يبدأ مسودة جديدة.'))}<section class="panel"><div class="card-title"><div><h2>${t('استيراد DOCX منظم')}</h2><p>${t('الرفع والتحقق يجريان على الخادم بحد حجم ونوع ملف.')}</p></div><span class="status-badge warning">${t('لا نشر تلقائي')}</span></div><form id="instrument-upload-form" class="form-grid"><label class="field full"><span>${t('ملف DOCX')}</span><input id="instrument-file" name="instrument" type="file" accept=".docx,application/vnd.openxmlformats-officedocument.wordprocessingml.document" required><small>${t('يجب أن يحتوي TABLE:DIMENSIONS وTABLE:ITEMS والجداول العلمية المرتبطة.')}</small></label><button class="primary-button full">${t('رفع والتحقق')}</button></form><div id="instrument-preview"></div></section><section class="panel"><div class="card-title"><div><h2>${t('حالتان منفصلتان')}</h2><p>${t('حالة المنتج قرار تشغيلي. الحالة العلمية ادعاء استدلالي لا يتغير تلقائيًا، ويحتاج مبررًا مكتوبًا ويُسجَّل في التدقيق.')}</p></div></div>${rows.length ? table([t('الإصدار'),t('حالة المنتج'),t('الحالة العلمية'),t('البنود'),t('التقييمات'),''],rows,6,940) : emptyState(t('لا توجد إصدارات'),t('ارفع ملف الأداة المنظم لإنشاء مسودة.'))}</section></div>`;
}
...
async function renderInstrument(route) {
  ...
  appView.innerHTML = `<div class="page">${pageHeading(`إصدار ${e(detail.version.version)}`,e(detail.version.name),`الحالة: ${e(detail.version.status)} · ${number(items.items.length)} بندًا.`,`<a class="secondary-button" href="#instruments">${t('كل الإصدارات')}</a>`)}${modelNotice()}<section class="panel"><h2>${t('الأبعاد')}</h2>${table([t('الرمز'),t('البناء'),t('الاسم'),t('الوزن')],dimRows,4,650)}</section><section class="panel"><h2>${t('بنك البنود')}</h2>${table([t('البند'),t('البناء'),t('الصياغة'),t('مطلوب'),t('عكسي')],itemRows,5,900)}</section></div>`;
}
...
async function renderDataQuality() {
  ...
  appView.innerHTML = `<div class="page">${pageHeading(t('البحث · الجودة'),t('جودة البيانات'),t('لا تُحذف الحالات آليًا؛ تظهر الإشارات للباحث للمراجعة.'),'<a class="secondary-button" href="#statistics">الإحصاءات الوصفية</a>')}<div class="card-grid"><article class="card metric-card span-4"><small>${t('إشارات الجودة')}</small><strong>${number(data.flags.length)}</strong></article><article class="card metric-card span-4"><small>${t('حالات غير مكتملة')}</small><strong>${number(data.incomplete_cases.length)}</strong></article><article class="card metric-card span-4"><small>${t('حذف تلقائي')}</small><strong>${number(data.auto_deleted)}</strong></article></div><section class="panel">${rows.length ? table([t('التقييم'),t('الإشارة'),t('الخطورة'),t('التفاصيل'),t('الحالة')],rows,5,820) : emptyState(t('لا توجد إشارات جودة'),t('لم يكتشف المحرك حالات تستدعي المراجعة.'))}</section></div>`;
}
...
async function renderStatistics(route) {
  ...
  const section = (title,item) => item ? `<section class="card span-6"><h2>${e(title)}</h2><div class="card-grid">${metric('N',item.n)}${metric(t('المتوسط'),item.mean)}${metric(t('الوسيط'),item.median)}${metric(t('الانحراف المعياري'),item.sd)}${metric(t('الأدنى'),item.min)}${metric(t('الأعلى'),item.max)}</div></section>` : `<section class="card span-6">${emptyState(`لا توجد بيانات ${title}`,t('لا توجد ملاحظات كافية للفلتر المختار.'))}</section>`;
  appView.innerHTML = `<div class="page">${pageHeading(t('البحث · الإحصاءات'),t('الإحصاءات الوصفية'),`مصدر البيانات: ${e(data.data_origin)}. لا تُعرض موثوقية أو صلاحية مصطنعة.`)}<div class="card-grid">${section('MCM',data.MCM)}${section('SMCE',data.SMCE)}<article class="card span-6"><h2>${t('الموثوقية')}</h2>${badge(t('غير متاح'),'neutral')}<p>${e(data.reliability.reason)}</p></article><article class="card span-6"><h2>${t('الصلاحية')}</h2>${badge(t('غير متاح'),'neutral')}<p>${e(data.validity.reason)}</p></article></div></div>`;
}

function renderExports() {
  appView.innerHTML = `<div class="page">${pageHeading(t('البحث · التصدير'),t('Excel وSPSS جاهزان للتحليل'),t('صف واحد لكل ملاحظة، أسماء متغيرات ASCII، قيم رقمية صحيحة، وترميزات وقاموس متغيرات منفصل—دون معرّفات شخصية مباشرة.'))}<div class="card-grid"><section class="panel span-7"><form id="export-form" class="form-grid"><label class="field"><span>${t('مصدر البيانات')}</span><select name="data_origin"><option value="REAL">${t('واقعية بموافقة بحثية')}</option><option value="SYNTHETIC">${t('اصطناعية فقط')}</option><option value="DEMO_TEST">Demo / Test</option><option value="ALL">${t('كل المصادر - مراجعة خاصة')}</option></select></label><label class="field"><span>${t('نوع التصدير')}</span><select name="format"><option value="XLSX">${t('Excel متوافق مع SPSS')}</option><option value="SPSS">${t('حزمة SPSS: XLSX + CSV + Syntax')}</option><option value="CSV">CSV UTF-8</option><option value="CODEBOOK">${t('قاموس المتغيرات Excel')}</option><option value="INSTRUMENT">Instrument JSON</option></select></label><label class="checkbox-field full"><input type="checkbox" required><span>${t('أفهم أن التصدير مسجّل في سجل التدقيق وأن البيانات الواقعية تتطلب موافقة بحثية.')}</span></label><button class="primary-button full">${t('إنشاء وتنزيل الملف')}</button></form></section><aside class="card span-5 export-summary"><h2>${t('تشمل المتغيرات')}</h2><ul><li>${t('خصائص المنشأة: القطاع والحجم والعمر والمنطقة.')}</li><li>${t('عدد المنصات ودور المجيب والممكنات الأربعة.')}</li><li>${t('بنود ليكرت ودرجات الأبعاد MCM وSMCE.')}</li><li>${t('المرحلة الخماسية وفارق الكفاءة عن النضج.')}</li></ul></aside></div><section class="card"><h2>${t('محتويات مصنف Excel')}</h2><div class="pill-list">${['01_RESPONSES_WIDE','02_RESPONSES_LONG','03_MCM_SCORES','04_SMCE_SCORES','05_COMPANY_PROFILE','06_CODEBOOK','07_VARIABLE_LABELS','08_VALUE_LABELS','09_INSTRUMENT','10_METADATA'].map(item => `<span>${item}</span>`).join('')}</div></section></div>`;
}
...
function correlationCell(result) {
  if (!result || !result.available) {
    return `<span class="corr-cell muted">${e(result?.notice_ar || t('غير متاح'))} (N=${number(result?.n || 0)})</span>`;
  }
...
function scatterPlot(points, boundaries) {
  ...
  return `<div class="scatter-wrap"><svg class="scatter" viewBox="0 0 ${size} ${size}" role="img" aria-label="${e(`خريطة النضج مقابل الكفاءة. ${described}`)}"><g class="scatter-grid">${grid}</g>${median}<g class="scatter-points">${dots}</g><g>${labels}</g><text class="axis-title" x="${size/2}" y="${size-6}" text-anchor="middle">${t('MCM · النضج')}</text><text class="axis-title" transform="rotate(-90 14 ${size/2})" x="14" y="${size/2}" text-anchor="middle">${t('SMCE · الكفاءة')}</text></svg></div>`;
}
...
async function renderAnalytics(route) {
  ...
  const groupSections = Object.entries(data.groups).map(([variable, block]) => {
    ...
    return `<section class="panel"><div class="card-title"><div><h2>${e(block.label_ar)}</h2><p>${t('وصف منفصل لكل من النضج والكفاءة داخل كل فئة.')}</p></div></div>${rows.length ? table([t('الفئة'),t('العدد'),'MCM','SMCE'],rows,4,760) : emptyState(t('لا توجد فئات'),t('لم تصل بيانات كافية بعد.'))}</section>`;
  }).join('');

  const q = data.quadrants;
  const quadrantCards = Object.entries(q.labels_ar).map(([key,label]) =>
    `<article class="card quadrant-card span-3"><small>${e(label)}</small><strong>${number(q.counts[key] || 0)}</strong><span>${t('منشأة')}</span></article>`).join('');

  const versionWarning = data.version_warning_ar ? `<div class="research-notice"><strong>${t('تنبيه إصدارات')}</strong><span>${e(data.version_warning_ar)}</span></div>` : '';

  appView.innerHTML = `<div class="page">${pageHeading(t('التحليل · مركز البيانات'),t('مركز تحليل البيانات'),t('إحصاءات وصفية وعلاقات بين المتغيرات، محسوبة على الخادم من الدرجات المخزّنة.'))}
    ${versionWarning}
    <div class="research-notice"><strong>${t('حدود القراءة')}</strong><span>${e(data.notice_ar)}</span></div>
    <section class="panel"><div class="card-title"><div><h2>${t('نطاق التحليل')}</h2></div></div><form id="analytics-filters" class="form-grid">
      <label class="field"><span>${t('وحدة التحليل')}</span><select name="mode"><option value="LATEST_PER_ORGANIZATION" ${current('mode') !== 'ALL_COMPLETED' ? 'selected':''}>${t('آخر تقييم لكل منشأة')}</option><option value="ALL_COMPLETED" ${current('mode') === 'ALL_COMPLETED' ? 'selected':''}>${t('كل التقييمات المكتملة')}</option></select></label>
      <label class="field"><span>${t('مصدر البيانات')}</span><select name="data_origin">${['REAL','TEST','DEMO','ALL'].map(v=>`<option value="${v}" ${(current('data_origin') || 'REAL') === v ? 'selected':''}>${e(v === 'REAL' ? t('واقعي') : v === 'ALL' ? t('الكل') : v)}</option>`).join('')}</select></label>
      <label class="field"><span>${t('القطاع')}</span><input name="sector" value="${e(current('sector'))}" placeholder="كل القطاعات"></label>
      <label class="field"><span>${t('حجم المنشأة')}</span><select name="firm_size"><option value="">${t('الكل')}</option>${['MICRO','SMALL','MEDIUM'].map(v=>`<option value="${v}" ${current('firm_size') === v ? 'selected':''}>${e({MICRO:t('متناهية الصغر'),SMALL:t('صغيرة'),MEDIUM:t('متوسطة')}[v])}</option>`).join('')}</select></label>
      <div class="field full button-row"><button class="primary-button" type="submit">${t('تطبيق')}</button><a class="secondary-button" href="#analytics">${t('إعادة الضبط')}</a></div>
    </form></section>
    <div class="card-grid">${cards}</div>
    <section class="panel"><div class="card-title"><div><h2>${t('خريطة النضج والكفاءة')}</h2><p>${t('كل نقطة منشأة واحدة. الخطان المتقطعان وسيط العينة، وهما حدّ الأرباع لا عتبة معيارية.')}</p></div></div>${scatterPlot(data.scatter, q)}<div class="card-grid">${quadrantCards}</div></section>
    <section class="panel"><div class="card-title"><div><h2>${t('العلاقة بين النضج والكفاءة')}</h2><p>${e(data.relationship.interpretation_ar)}</p></div></div><div class="correlation-headline">${correlationCell(data.relationship)}</div></section>
    <section class="panel"><div class="card-title"><div><h2>${t('توزيع المراحل')}</h2></div></div>${stageRows.length ? table([t('المرحلة'),t('العدد'),t('النسبة')],stageRows,3,520) : emptyState(t('لا توجد حالات مصنفة'),'')}</section>
    <section class="panel"><div class="card-title"><div><h2>${t('وصف الأبعاد')}</h2><p>${t('الأبعاد السبعة للنضج والخمسة للكفاءة والممكنات والنتائج.')}</p></div></div>${dimensionRows.length ? table([t('البعد'),t('المتوسط'),t('الوسيط'),t('الانحراف'),t('المدى'),'N'],dimensionRows,6,760) : emptyState(t('لا توجد درجات أبعاد'),'')}</section>
    <section class="panel"><div class="card-title"><div><h2>${t('الارتباط بين المتغيرات')}</h2><p>يُعرض المعامل فقط عند بلوغ ${number(data.minimum_relationship_sample)} حالة. الارتباط وصفي ولا يثبت سببية.</p></div></div>${associationRows.length ? table([t('المتغير'),t('وصفه'),t('مع MCM'),t('مع SMCE')],associationRows,4,900) : emptyState(t('لا توجد متغيرات'),'')}</section>
    ${groupSections}
  </div>`;
}
...
async function renderConsultations(route) {
  ...
  const rows = data.leads.map(lead => {
    ...
    const actions = withheld ? '' : `<button class="secondary-button" data-action="consultation-status" data-id="${lead.id}" data-current="${e(lead.status)}">${t('الحالة')}</button><button class="secondary-button" data-action="consultation-note" data-id="${lead.id}">${t('ملاحظة')}</button>${isAdmin ? `<button class="secondary-button" data-action="consultation-assign" data-id="${lead.id}" data-current="${lead.assigned_consultant_id || ''}">${t('إسناد')}</button>` : ''}`;
    return `<div class="data-table__row">
      ...
      <span>${(lead.consultation_topics || []).map(topic=>e(consultationTopicLabels[topic] || topic)).join(t('، ')) || '--'}</span>
      <span>${badge(lead.status, lead.status === 'DO_NOT_CONTACT' ? 'danger' : lead.status === 'NEW' ? 'warning' : '')}</span>
      <span>${e(lead.consultant_name || t('غير مسند'))}</span>
      <span>${dateText(lead.created_at)}</span>
      <span class="button-row"><a class="secondary-button" href="#results/${lead.assessment_id}">${t('النتيجة')}</a>${actions}${isAdmin ? `<button class="danger-button" data-action="consultation-erase" data-id="${lead.id}">${t('حذف البيانات')}</button>` : ''}</span>
    </div>`;
  });
  const current = key => route?.query?.get(key) || '';
  const filterBar = `<form id="consultation-filters" class="form-grid">
    <label class="field"><span>${t('الحالة')}</span><select name="status"><option value="">${t('الكل')}</option>${data.statuses.map(value=>`<option value="${value}" ${current('status') === value ? 'selected':''}>${e(statusLabels[value] || value)}</option>`).join('')}</select></label>
    <label class="field"><span>${t('المحور')}</span><select name="topic"><option value="">${t('الكل')}</option>${(data.topics || []).map(value=>`<option value="${value}" ${current('topic') === value ? 'selected':''}>${e(consultationTopicLabels[value] || value)}</option>`).join('')}</select></label>
    ${isAdmin ? `<label class="field"><span>${t('المستشار')}</span><select name="assigned_consultant_id"><option value="">${t('الكل')}</option>${(data.consultants || []).map(item=>`<option value="${item.id}" ${current('assigned_consultant_id') === String(item.id) ? 'selected':''}>${e(item.name)}</option>`).join('')}</select></label>` : ''}
    <label class="field"><span>${t('من تاريخ')}</span><input name="created_from" type="date" value="${e(current('created_from_date'))}"></label>
    <div class="field full button-row"><button class="primary-button" type="submit">${t('تطبيق الفلاتر')}</button><a class="secondary-button" href="#consultations">${t('مسح')}</a></div>
  </form>`;
  const scopeNotice = isAdmin ? '' : '<div class="research-notice"><strong>نطاقك</strong><span>تُعرض الطلبات المسندة إليك فقط.</span></div>';
  appView.innerHTML = `<div class="page">${pageHeading(isAdmin ? t('إدارة المنصة · الاستشارات') : t('مساحة العمل · الاستشارات'),t('طلبات الاستشارة'),t('بيانات التواصل تُعرض لمن يملك صلاحية معلنة فقط، وتُحجب فور سحب الموافقة، ولا تدخل في أي تصدير بحثي.'))}${scopeNotice}<div class="card-grid">${cards}</div><section class="panel"><div class="card-title"><div><h2>${t('تصفية')}</h2></div></div>${filterBar}</section><section class="panel">${rows.length ? table([t('الطلب'),t('المنشأة'),t('التواصل'),'MCM','SMCE',t('المحاور'),t('الحالة'),t('المستشار'),t('التاريخ'),''],rows,10,1520) : emptyState(t('لا توجد طلبات مطابقة'),t('عدّل الفلاتر أو انتظر وصول طلب جديد من صفحة النتائج.'))}</section></div>`;
}
...
async function renderAdmin() {
  ...
  const userRows = users.users.map(item => {
    const memberships = (item.memberships || []).map(membership => `<li>
      ...
      <button class="danger-button" data-action="remove-membership" data-user="${item.id}" data-org="${membership.organization_id}" data-name="${e(item.name || '')}" data-org-name="${e(membership.organization_name || '')}" type="button">${t('إزالة من المؤسسة')}</button>
    ...
    return `<div class="data-table__row user-row">
      ...
      <span>${badge(item.is_active ? t('نشط'):t('معطل'), item.is_active ? '':'danger')}</span>
      <span>${number((item.memberships || []).length)} عضوية</span>
      <span class="membership-list"><ul>${memberships}</ul></span>
      <span class="button-row">
        <button class="secondary-button" data-action="toggle-user" data-id="${item.id}" data-active="${item.is_active ? '1':'0'}">${item.is_active ? t('تعطيل'):t('تفعيل')}</button>
        <button class="danger-button" data-action="delete-user" data-id="${item.id}" data-name="${e(item.name || '')}" data-email="${e(item.email || '')}">${t('حذف الحساب')}</button>
      </span>
    </div>`;
  });
  const options = organizations.organizations.map(item => `<option value="${item.id}">${e(item.name)}</option>`).join('');
  appView.innerHTML = `<div class="page">${pageHeading(t('إدارة المنصة'),t('لوحة المدير العام'),t('إدارة المستخدمين والمنظمات والإعدادات التشغيلية.'))}<div class="card-grid"><article class="card metric-card span-3"><small>${t('المؤسسات')}</small><strong>${number(overview.organizations)}</strong></article><article class="card metric-card span-3"><small>${t('المستخدمون')}</small><strong>${number(overview.users)}</strong></article><article class="card metric-card span-3"><small>${t('التقييمات')}</small><strong>${number(overview.assessments)}</strong></article><article class="card metric-card span-3"><small>${t('الجلسات النشطة')}</small><strong>${number(overview.active_sessions)}</strong></article><section class="card span-12"><div class="card-title"><div><h2>${t('إنشاء مستخدم')}</h2><p>${t('تعيين مؤسسة ودور واضحين.')}</p></div></div><form id="admin-user-form" class="form-grid"><label class="field"><span>${t('الاسم')}</span><input name="name" required></label><label class="field"><span>${t('البريد')}</span><input name="email" type="email" required></label><label class="field"><span>${t('كلمة المرور')}</span><input name="password" type="password" minlength="10" required></label><label class="field"><span>${t('المؤسسة')}</span><select name="organization_id">${options}</select></label><label class="field full"><span>${t('الدور')}</span><select name="role"><option value="COMPANY_RESPONDENT">${t('مشارك شركة')}</option><option value="COMPANY_ADMIN">${t('مدير شركة')}</option><option value="CONSULTANT">${t('استشاري')}</option><option value="RESEARCHER">${t('باحث')}</option><option value="SUPER_ADMIN">${t('مدير المنصة')}</option></select></label><button class="primary-button full">${t('إنشاء المستخدم')}</button></form></section></div><section class="panel"><div class="card-title"><div><h2>${t('الحسابات والعضويات')}</h2><p>${t('الحساب واحد وقد ينتمي لعدة مؤسسات. إزالة العضوية تُلغي وصوله لتلك المؤسسة وحدها؛ حذف الحساب يُلغيه بالكامل ولا يمس تقييمات المؤسسات.')}</p></div></div>${table([t('المستخدم'),t('الحالة'),t('العضويات'),t('المؤسسات والأدوار'),''],userRows,5,1080)}</section><section class="card"><h2>${t('إعدادات النظام')}</h2><div class="pill-list">${Object.entries(overview.configuration).map(([key,value]) => `<span>${e(key)}: ${e(typeof value === 'object' ? JSON.stringify(value) : value)}</span>`).join('')}</div></section></div>`;
}
...
document.addEventListener('click', async event => {
  ...
  try {
    if (action === 'toggle-language') {
      ...
      openDialog(`<h2 id="dialog-title">${t('تغيير الحالة العلمية')}</h2><p class="classification-boundary">${t('هذا ادعاء استدلالي عن الإصدار، ومستقل عن حالة المنتج. لا يُرفع تلقائيًا، ويُسجَّل كل تغيير في سجل التدقيق باسمك.')}</p><form id="scientific-status-form" class="form-grid"><input type="hidden" name="version_id" value="${e(button.dataset.id)}"><label class="field full"><span>${t('الحالة العلمية')}</span><select name="scientific_status" required>${options.map(value=>`<option value="${value}" ${button.dataset.current === value ? 'selected':''}>${e(statusLabels[value] || value)}</option>`).join('')}</select></label><label class="field full"><span>${t('المبرر (إلزامي)')}</span><textarea name="rationale" rows="3" required minlength="10" placeholder="ما الدليل الذي يسند هذه الحالة؟"></textarea></label><button class="primary-button full" type="submit">${t('حفظ الحالة العلمية')}</button></form>`);
    }
      ...
      if (button.dataset.participant === '1') {
        ...
        else { sessionStorage.removeItem('mcm_participant_token'); appView.innerHTML = `<section class="route-state">${emptyState(t('شكرًا لمشاركتك'),t('تم إرسال التقييم وقفل إجاباتك بنجاح.'),'<a class="primary-button" href="#landing">إنهاء</a>')}</section>`; }
      } else {
      ...
      openDialog(`<div class="ai-plan-dialog"><span class="status-badge">${e(modeLabel)}</span><h2 id="dialog-title">${t('خطة التحسين المقترحة')}</h2><p>${e(plan.summary_ar)}</p><div class="ai-plan-priorities">${(plan.priorities || []).map(item=>`<span><b>${e(item.dimension_code)}</b>${e(item.name_ar)} · ${number(item.score,1)}/100</span>`).join('')}</div>${roadmap}<p class="classification-boundary">${e(plan.safeguards?.notice_ar || t('تحتاج الخطة مراجعة واعتمادًا بشريًا قبل التنفيذ.'))}</p><button class="secondary-button" data-action="close-dialog">${t('إغلاق')}</button></div>`);
      ...
      openDialog(`<h2 id="dialog-title">حذف التقييم #${e(button.dataset.id)}</h2><p class="classification-boundary">${t('سيُحذف التقييم وكل إجاباته ودرجاته وتشخيصاته وخارطته وتقاريره نهائيًا. لا يمكن التراجع، وسيتأثر أي تحليل بحثي يعتمد على هذه الحالة.')}</p><div class="button-row"><button class="danger-button" data-action="confirm-delete-assessment" data-id="${e(button.dataset.id)}" type="button">${t('تأكيد الحذف النهائي')}</button><button class="secondary-button" data-action="close-dialog">${t('إلغاء')}</button></div>`);
    }
      ...
      openDialog(`<h2 id="dialog-title">${t('كود مشاركة الزملاء')}</h2><p>${t('شارك هذا الكود مع زملائك في المنشأة ليجيبوا على نفس التقييم. تُجمع إجاباتهم في تقييم واحد ولا يلزمهم حساب.')}</p><div class="join-code-display" dir="ltr">${e(code)}</div><label class="field full"><span>${t('أو شارك الرابط')}</span><input id="join-url" dir="ltr" value="${e(url)}" readonly></label><p class="consultation-note">صالح حتى ${e(dateText(result.join_code.expires_at))} · حد الاستخدام ${number(result.join_code.max_uses)} · استُخدم ${number(result.join_code.use_count)}</p><div class="button-row"><button class="primary-button" type="button" data-action="copy-join-url">${t('نسخ الرابط')}</button><button class="secondary-button" data-action="close-dialog">${t('إغلاق')}</button></div>`);
    }
      ...
      openDialog(`<h2 id="dialog-title">تحديث حالة الطلب #${e(button.dataset.id)}</h2><form id="consultation-status-form" class="form-grid"><input type="hidden" name="lead_id" value="${e(button.dataset.id)}"><label class="field full"><span>${t('الحالة')}</span><select name="status" required>${statuses.map(value=>`<option value="${value}" ${button.dataset.current === value ? 'selected':''}>${e(statusLabels[value] || value)}</option>`).join('')}</select></label><label class="field full"><span>${t('ملاحظة داخلية')}</span><textarea name="note" rows="2" maxlength="2000"></textarea></label><label class="field full"><span>${t('سبب الإغلاق (عند الإغلاق أو الإيقاف)')}</span><input name="closure_reason" maxlength="400"></label><div class="form-error full" hidden></div><button class="primary-button full" type="submit">${t('حفظ الحالة')}</button></form>`);
    }
    else if (action === 'consultation-note') {
      openDialog(`<h2 id="dialog-title">ملاحظة داخلية على الطلب #${e(button.dataset.id)}</h2><p>${t('تُسجَّل الملاحظة في سجل الطلب باسمك ولا تظهر للمشارك.')}</p><form id="consultation-note-form" class="form-grid"><input type="hidden" name="lead_id" value="${e(button.dataset.id)}"><label class="field full"><span>${t('الملاحظة')}</span><textarea name="note" rows="3" required maxlength="2000"></textarea></label><div class="form-error full" hidden></div><button class="primary-button full" type="submit">${t('إضافة')}</button></form>`);
    }
    else if (action === 'consultation-assign') {
      const board = await api('/api/admin/consultations');
      openDialog(`<h2 id="dialog-title">إسناد الطلب #${e(button.dataset.id)}</h2><form id="consultation-assign-form" class="form-grid"><input type="hidden" name="lead_id" value="${e(button.dataset.id)}"><label class="field full"><span>${t('المستشار')}</span><select name="consultant_id" required>${(board.consultants || []).map(item=>`<option value="${item.id}" ${String(button.dataset.current) === String(item.id) ? 'selected':''}>${e(item.name)}</option>`).join('')}</select></label><label class="field full"><span>${t('ملاحظة')}</span><input name="note" maxlength="400"></label><div class="form-error full" hidden></div><button class="primary-button full" type="submit">${t('إسناد')}</button></form>`);
    }
    else if (action === 'consultation-erase') {
      openDialog(`<h2 id="dialog-title">${t('حذف بيانات التواصل')}</h2><p class="classification-boundary">${t('يُنفَّذ طلب الحذف: تُمحى بيانات التواصل نهائيًا ويبقى سجل الطلب لأغراض التدقيق دون البيانات الشخصية. لا يمكن التراجع.')}</p><div class="button-row"><button class="danger-button" data-action="confirm-consultation-erase" data-id="${e(button.dataset.id)}" type="button">${t('تأكيد الحذف')}</button><button class="secondary-button" data-action="close-dialog">${t('إلغاء')}</button></div>`);
    }
...
document.addEventListener('submit', async event => {
  ...
  try {
    if (form.id === 'auth-form') {
      ...
      const dev=result.development_reset_token ? `<p class="form-success">${t('بيئة التطوير:')} <a href="#reset?token=${e(result.development_reset_token)}">${t('فتح رابط الاستعادة')}</a></p>`:'';
      ...
      try {
        ...
        openDialog(`<div class="consultation-success"><span class="status-badge">${t('تم الاستلام')}</span><h2 id="dialog-title">${response.duplicate ? t('طلبك مسجّل بالفعل') : t('تم استلام طلب الاستشارة')}</h2><p>${e(response.confirmation_ar || t('تم استلام طلب الاستشارة، وسيتواصل معك الفريق وفق وسيلة التواصل التي اخترتها.'))}</p><div class="pill-list"><span>رقم الطلب: ${number(lead.id)}</span><span>الحالة: ${e(statusLabels[lead.status] || lead.status)}</span></div><p class="consultation-note">${t('احتفظ برقم الطلب. يمكنك سحب موافقة التواصل لاحقًا عبر')} <a href="#privacy">${t('سياسة الخصوصية')}</a> ${t('دون أن يؤثر ذلك على نتيجتك.')}</p><div class="button-row"><button class="primary-button" data-action="close-dialog">${t('إغلاق')}</button></div></div>`);
      } catch (error) {
      ...
      preview.innerHTML=`<div class="card ${result.valid?'':'diagnosis-card high'}"><h3>${result.valid?t('اجتاز الملف التحقق'):t('الملف يحتاج تصحيحًا')}</h3><div class="pill-list"><span>${number(result.stats.items)} بندًا</span><span>${number(result.stats.dimensions)} أبعاد</span><span>${number(result.stats.mcm_items)} MCM</span><span>${number(result.stats.smce_items)} SMCE</span></div>${result.errors.length?`<ul>${result.errors.map(item=>`<li>${e(item.code)} ${e(item.table||item.item||'')}</li>`).join('')}</ul>`:''}${result.warnings.length?`<p>تحذيرات: ${result.warnings.map(item=>e(item.table||item.code)).join(t('، '))}</p>`:''}${result.valid?'<button class="primary-button" data-action="import-instrument" type="button">إنشاء مسودة غير منشورة</button>':''}</div>`;
    }
...
document.querySelector('#user-menu').addEventListener('click',()=>openDialog(`<h2 id="dialog-title">${e(state.me?.user?.name||t('الحساب'))}</h2><p>${e(state.me?.user?.email||'')}</p><div class="button-row"><button class="secondary-button" data-action="open-settings">${t('الإعدادات')}</button><button class="danger-button" data-action="logout">${t('تسجيل الخروج')}</button></div>`));
