...
const errorLabels = {
  ...
  invalid_demographics: 'تحقق من عمر المنشأة وعدد منصات التواصل.', enablers_required: 'أجب عن الممكنات التنظيمية الأربعة.',
  invalid_enabler_rating: 'اختر إجابة لفظية لكل عامل تمكيني.', platform_not_ready: 'المنصة قيد التهيئة؛ حاول بعد قليل.',
};
...
function renderParticipantStart() {
  ...
  appView.innerHTML = `<div class="participant-start-page">${publicNav()}<main><section class="start-heading"><div><span>دخول مباشر للمشارك</span><h1>عرّفنا بمنشأتك قبل بدء المقياس</h1><p>هذه المتغيرات ضرورية لتفسير النتيجة ومقارنة المنشآت الصغيرة والمتوسطة بصورة سليمة، لكنها لا تدخل في درجة MCM.</p></div><div class="start-meta"><b>حوالي 12–15 دقيقة</b><span>لا يلزم إنشاء حساب</span><span>الحفظ تلقائي</span></div></section>${storageNotice}<form id="direct-assessment-form" class="direct-form"><section class="panel"><div class="form-section-title"><b>01</b><div><h2>الخصائص الديموغرافية للمنشأة</h2><p>تُحفظ مع حالة القياس لأغراض التشخيص والتحليل المجهول وفق الموافقات.</p></div></div><div class="form-grid"><label class="field full"><span>اسم المنشأة</span><input name="organization_name" minlength="2" maxlength="120" required></label><label class="field"><span>القطاع</span><select name="sector" required><option value="">اختر القطاع</option><option>التجزئة والتجارة</option><option>الخدمات المهنية</option><option>التقنية والاتصالات</option><option>الصناعة</option><option>السياحة والضيافة</option><option>الصحة</option><option>التعليم</option><option>النقل والخدمات اللوجستية</option><option>قطاع آخر</option></select></label><label class="field"><span>حجم المنشأة</span><select name="firm_size" required><option value="">اختر الحجم</option><option value="MICRO">متناهية الصغر</option><option value="SMALL">صغيرة</option><option value="MEDIUM">متوسطة</option></select></label><label class="field"><span>عمر المنشأة بالسنوات</span><input name="firm_age_years" type="number" min="0" max="200" required></label><label class="field"><span>المنطقة</span><select name="region" required><option value="">اختر المنطقة</option>${regions.map(region=>`<option>${e(region)}</option>`).join('')}</select></label><label class="field"><span>عدد منصات التواصل المستخدمة</span><input name="social_platform_count" type="number" min="0" max="50" required></label><label class="field"><span>دور المجيب</span><select name="respondent_role" required><option value="">اختر الدور</option><option>مالك / مؤسس</option><option>إدارة عليا</option><option>مدير تسويق أو اتصال</option><option>أخصائي تسويق أو اتصال</option><option>خدمة عملاء / تجربة عميل</option><option>دور آخر</option></select></label></div></section><section class="panel"><div class="form-section-title"><b>02</b><div><h2>الممكنات التنظيمية والسياق</h2><p>قيّم وضع المنشأة الحالي باستخدام خيارات ليكرت اللفظية.</p></div></div><div class="context-likert-grid">${labelledLikert('leadership_support','تدعم القيادة تطوير الاتصال التسويقي بوضوح.')}${labelledLikert('human_competencies','تتوفر الكفاءات البشرية اللازمة لتنفيذ الاتصال وتطويره.')}${labelledLikert('technology_infrastructure','تدعم البنية التقنية جمع البيانات وتبادلها بأمان.')}${labelledLikert('data_readiness','تتوفر بيانات محدثة وموثوقة وجاهزة للقرار الاتصالي.')}</div></section><section class="panel consent-panel"><label class="checkbox-field"><input name="service_consent" type="checkbox" required><span><b>موافقة الخدمة مطلوبة:</b> أوافق على معالجة الإجابات لإصدار التشخيص والتقرير للمنشأة.</span></label><label class="checkbox-field"><input name="research_consent" type="checkbox"><span><b>موافقة بحثية اختيارية:</b> أوافق على استخدام البيانات بعد إزالة المعرّفات المباشرة لأغراض البحث والمقارنة.</span></label><div class="form-error" hidden></div><button class="primary-button large" type="submit">حفظ البيانات وبدء المقياس</button></section></form></main></div>`;
}

function renderPublicResult(result) {
  state.publicPage = true; applyShell();
  const mcm = result.scores.MCM; const smce = result.scores.SMCE; const relation = result.relationship || {};
  const stages = result.maturity_progression || [];
  const dimensions = [...mcm.dimensions.map(item=>({...item,tone:'navy'})),...smce.dimensions.map(item=>({...item,tone:'teal'}))];
  appView.innerHTML = `<div class="public-result-page">${publicNav()}<main><section class="result-hero"><span>تم إكمال المقياس بنجاح</span><h1>تصنيف منشأتك: <i>${e(mcm.maturity_level?.label_ar || 'غير متاح')}</i></h1><p>${e(relation.narrative_ar || '')}</p><div class="result-score-cards"><article><small>النضج MCM</small><strong>${number(mcm.total,1)}</strong><span>/100</span></article><article><small>الكفاءة SMCE</small><strong>${number(smce.total,1)}</strong><span>/100</span></article><article><small>فارق الكفاءة عن النضج</small><strong>${Number(relation.efficiency_minus_maturity || 0) > 0 ? '+' : ''}${number(relation.efficiency_minus_maturity,1)}</strong><span>نقطة</span></article></div></section><section class="panel"><div class="card-title"><div><h2>المراحل الخمس</h2><p>المرحلة الحالية مميزة ضمن مسار التطور التشخيصي.</p></div></div><div class="maturity-stages compact">${stages.map(stage=>`<div class="${stage.level_order === mcm.maturity_level?.order ? 'active':''}"><b>${number(stage.level_order)}</b><span>${e(stage.label_ar)}</span></div>`).join('')}</div></section><section class="card-grid"><article class="card span-6"><h2>ربط النضج بالكفاءة</h2><div class="relationship-visual"><b>MCM</b><span>أثر إيجابي مقترح ←</span><b>SMCE</b></div><p>${e(relation.interpretation_ar || '')}</p><div class="research-notice"><strong>حدود التفسير</strong><span>${e(result.classification_notice)}</span></div></article><article class="card span-6"><h2>ملف الأبعاد</h2><div class="dimension-list">${dimensions.map(item=>`<div class="dimension-line ${item.tone}"><span>${e(item.code)} · ${e(item.name)}</span><div class="dimension-track"><i style="--score:${Number(item.score)}%"></i></div><b>${number(item.score,1)}</b></div>`).join('')}</div></article></section><div class="button-row public-result-actions"><button class="secondary-button" data-action="print-result">طباعة النتيجة</button><a class="primary-button" href="#landing">العودة للرئيسية</a></div></main></div>`;
}
...
function questionCard(item, response, disabled = false) {
  ...
  if (type === 'TEXT') {
    ...
    const likertLabels = {1:'لا أوافق بشدة',2:'لا أوافق',3:'محايد',4:'أوافق',5:'أوافق بشدة'};
    const choices = values.map((choice,index) => `<label><input type="radio" name="q_${item.id}" value="${choice}" data-answer-input data-item-id="${item.id}" ${Number(value) === choice && !response?.missing_type ? 'checked' : ''}${lock}><span>${type === 'BOOLEAN' ? (index ? 'نعم' : 'لا') : e(likertLabels[choice] || String(choice))}</span></label>`).join('');
    control = `<div class="choice-scale labelled" style="--choice-count:${values.length}">${choices}</div>`;
  }
...
async function renderParticipantAssessment(route) {
  ...
  const visibleItems = data.context ? data.items.filter(item => item.construct !== 'ENABLER') : data.items;
  const byConstruct = visibleItems.reduce((acc,item) => ((acc[item.construct] ||= []).push(item),acc),{});
  const constructTitles = {MCM:['النضج الاتصالي التسويقي MCM','قيّم الممارسات المؤسسية الحالية عبر الأبعاد السبعة.'],SMCE:['كفاءة التواصل الاجتماعي SMCE','قيّم نتائج الاتصال اليومية عبر الأبعاد الخمسة.'],ENABLER:['الممكنات التنظيمية','تُحفظ كسياق ولا تدخل في درجة MCM.'],OUTCOME:['النتائج البعيدة الاختيارية','الثقة والرضا والعلامة وأثر الأعمال؛ لا تدخل في الدرجة الأساسية.']};
  ...
  appView.innerHTML = `<div class="page assessment-page participant-measure">${pageHeading(`تقييم #${data.assessment.id}`,'مقياس النضج الاتصالي التسويقي','اختر العبارة اللفظية الأقرب إلى واقع المنشأة الآن، وليس الوضع المأمول.')} ${researchNotice()}<section class="assessment-header"><div class="progress-row"><strong>التقدم <span id="progress-value">${number(data.review.progress)}%</span></strong><span class="save-state" id="save-state">محفوظ</span></div><div class="progress-track"><span id="progress-bar" style="--progress:${data.review.progress}%"></span></div></section><form id="assessment-form">${sections}</form><section class="panel submit-panel"><p>راجع البنود المطلوبة قبل الإرسال. بعد الإرسال ستُقفل الإجابات وتظهر النتيجة التشخيصية.</p><button class="primary-button large" data-action="submit-participant" type="button">إرسال التقييم وعرض النتيجة</button></section></div>`;
}
  ...
  try {
    if (form.id === 'auth-form') {
      ...
      const payload = {
        ...
        firm_age_years:Number(data.firm_age_years),region:data.region,social_platform_count:Number(data.social_platform_count),
        respondent_role:data.respondent_role,service_consent:fd.has('service_consent'),research_consent:fd.has('research_consent'),
        enablers:{leadership_support:Number(data.leadership_support),human_competencies:Number(data.human_competencies),technology_infrastructure:Number(data.technology_infrastructure),data_readiness:Number(data.data_readiness)},
      };
