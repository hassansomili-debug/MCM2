...
function modelNotice() {
  ...
  return `<div class="research-notice"><strong>${e(model.label_ar || 'نموذج تطبيقي قائم على الأدلة العلمية')}</strong><span>${e(model.result_disclaimer_ar || 'النتيجة تشخيصية وتطويرية، ولا تمثل شهادة اعتماد أو حكمًا نهائيًا على أداء المنشأة.')}</span></div>`;
}
...
function stageCards(levels) {
  return levels.map(level => `<li class="stage-card">
    ...
    <button class="stage-more" type="button" data-action="stage-detail" data-code="${e(level.code)}" aria-label="تعرف على مرحلة ${e(level.label_ar)}">تعرف على هذه المرحلة</button>
  </li>`).join('');
}
...
function stageDetailDialog(code) {
  ...
  openDialog(`<div class="stage-detail">
    ...
    <button class="secondary-button" data-action="close-dialog">إغلاق</button>
  </div>`);
}
...
async function renderLanding() {
  ...
  appView.innerHTML = `<div class="landing-page">${publicNav()}${storageNotice}<main>
    ...
    <section class="landing-section stages-section" id="maturity-public">
      <div class="section-intro"><span>مسار التطور</span><h2>${e(maturity.title_ar || 'مراحل النضج الاتصالي التسويقي')}</h2><p>${e(maturity.subtitle_ar || '')}</p></div>
      ...
      <div class="stages-cta"><a class="primary-button large" href="#participant-start">اكتشف مستوى منشأتك</a></div>
    </section>
    <section class="landing-section report-preview">
      <div><span>نتيجة قابلة للاستخدام</span><h2>نتائج تنفيذية، وليست مجرد درجة.</h2><p>يقارن بين MCM وSMCE، يعرض الأبعاد والممكنات والنتائج، يحدد المرحلة الحالية، ثم يرتب فرص التطوير. وتتوفر للمدير ملفات Excel وSPSS مع قاموس متغيرات وترميزات جاهزة.</p><a class="primary-button large" href="#participant-start">ابدأ تشخيص منشأتك</a></div>
      <div class="preview-card"><header><span>نتيجة افتراضية للتوضيح</span><b>${e(levels[2]?.label_ar || 'مدار / متكامل')}</b></header><div class="preview-score"><strong>64</strong><small>/100</small></div><div class="preview-bars">${[82,71,64,58,46].map((score,index)=>`<div><span>البعد ${index+1}</span><i><b style="--score:${score}%"></b></i><em>${score}</em></div>`).join('')}</div><footer><span>الأولوية التالية</span><b>تعزيز التعلّم والتكيّف</b></footer></div>
    </section>
  ...
  </main><footer class="public-footer"><b>مقياس النضج الاتصالي التسويقي</b><span>${e(model.label_ar || 'نموذج تطبيقي قائم على الأدلة العلمية')} للمنشآت الصغيرة والمتوسطة · 2026</span><a href="#login">تسجيل الدخول</a></footer></div>`;
}
...
async function renderKnowledgeAdmin() {
  ...
  appView.innerHTML = `<div class="page">${pageHeading('البحث · المعرفة','محرر المحتوى المعرفي',`المحتوى الذي يظهر في صفحة المعرفة العامة. لا يُولَّد آليًا ولا يُملأ بنص افتراضي. المكتوب ${number(data.written_count)} من ${number(data.dimension_count)}.`)}<div class="research-notice"><strong>ملاحظة</strong><span>${e(data.authoring_notice_ar)}</span></div>${sections || emptyState('لا توجد أبعاد','لم يُبذر إصدار أداة بعد.')}</div>`;
}
